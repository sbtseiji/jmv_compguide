```{r, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
  ...
  out <- jmvReadWrite::write_omv(data01, "data/omv/intro_data01.omv", frcWrt = TRUE)

```

# jamoviの導入 {#sec-ch-introduction}

まず，jamoviのインストール方法や起動方法，環境設定など，基本的な操作について見ていきます。

## jamoviのインストール {#sec-intro-install}

まずはソフトウェアのインストールからです。jamoviウェブサイトの[ダウンロードページ](https://www.jamovi.org/download.html)から，ソフトウェアをダウンロードしましょう。ダウンロードページには「2.x.xx solid」というダウンロードボタンと「2.x.xx current」というダウンロードボタンの2種類があります（[@fig-jamovi-download]）。
...
```{r fig-jamovi-download, fig.cap='jamoviのダウンロード画面（Mac版）の例', echo=FALSE}
...
jamoviを起動すると[@fig-jamovi-screen]のような画面が現れます。

```{r fig-jamovi-screen, fig.cap='jamoviのアプリケーション画面', echo=FALSE}
knitr::include_graphics("images/intro/jamovi-overview.png")
```

基本的な使い方については「[次章](#sec-ch-basics)」で説明しますので，ここでは起動できるかどうかだけ確認しておきましょう。
...
画面最上部，青色の帯の一番右端にある「**⋮**」をクリックすると，ウィンドウ右側にjamoviの画面表示や動作に関する設定項目が表示されます（[@fig-options-menu]）。


```{r fig-options-menu, fig.cap='環境設定メニュー', echo=FALSE }
knitr::include_graphics("images/intro/options-menu.png")
```

実際の使用場面では環境設定を変更する必要はほとんどないと思いますが，どのような設定項目があるのかをひととおり見ておくことにしましょう（[@fig-options-window]）。

```{r fig-options-window, fig.cap='環境設定項目', echo=FALSE }
knitr::include_graphics("images/intro/options.png")
```

これらの項目のうち，数値書式と*p値*の書式については説明が必要かもしれません（[@fig-options-number-format]）。


```{r fig-options-number-format, fig.cap='数値表示形式の設定', echo=FALSE}
...
環境設定用の「**⋮**」の隣にあるアイコンをクリックすると，分析結果の表示画面を全画面表示にしたり隠したりすることができます（[@fig-fullscreen-hide]）。できるだけ広い画面で入力したデータを確認したい場合などには，このボタンをクリックして分析結果を非表示にするとよいでしょう。

```{r fig-fullscreen-hide, fig.cap='結果ウィンドウの拡大・非表示', echo=FALSE}
...
jamoviでデータファイルを読み込んだり作業内容を保存したりする場合には，画面左上の「**≡**」を使用します（[@fig-file-menu]）。

```{r fig-file-menu, fig.cap='ファイルメニュー', echo=FALSE}
...
「**開く**」メニューは，すでにあるjamoviファイルやデータファイルを開く際に使用します（[@fig-file-open]）。

```{r fig-file-open, fig.cap='「開く」メニュー',echo=F, fig.align='center'}
...
「**拡張読み込み**」のメニューは，いま作業中のファイルに別のデータファイルの内容を追加する場合に使用します。「**開く**」とよく似ているのですが，「**拡張読み込み**」の場合には現在開いているデータファイルの内容が読み込んだファイルの内容に更新される点が異なります。なお，開いているデータファイルと「拡張読み込み」したファイルで変数名が異なる場合には，それらは新たな*変数*としてデータファイルに追加されます。
...
「`r infig('place-browse')` ブラウズ」をクリックします（[@fig-file-exercise]）。

```{r fig-file-exercise, fig.cap='ファイルを開く手順',echo=F, fig.align='center'}
...
[@fig-file-exercise-done]のような画面が表示されたら完了です。

```{r fig-file-exercise-done, fig.cap='データファイルを開いた様子',echo=F, fig.align='center'}
...
なお，テキストファイルやExcelファイルを使用する場合，データファイルの1行目は変数名とみなされます。実際のデータは2行目以降に入力されている必要がありますので注意してください。また，テキストファイルでは，各データ値の区切りには半角のコンマ（`,`）を使用します（[@fig-file-exercise-example]）。

```{r fig-file-exercise-example, fig.cap='データファイルの入力例',echo=F, fig.align='center'}
