...
jamoviの画面は，次の要素で構成されています（[@fig-basics-jamovi-overview]）。

```{r fig-basics-jamovi-overview, fig.cap="jamoviの画面構成",echo=F, fig.align="center"}
...
変数タブは，次のような画面構成になっています（[@fig-variable-overview]）。

```{r fig-variable-overview, fig.cap="変数タブ",echo=F, fig.align="center"}
...
データタブはデータの入力や編集の際に使用します。見た目はExcelなどの表計算ソフトによく似ています（[@fig-data-overview]）。

```{r fig-data-overview, fig.cap="データタブ",echo=F, fig.align="center"}
...
+ **変数**　変数の設定や変数値の処理に使用するツール群です。
    ...
    - `r infig("menu-compute")` 計算　すでにある変数値から新たな変数を作成します。
    - `r infig("menu-data-transform")` 変換　すでにある変数値に変換を適用して新たな変数を作成します。
    - `r infig("menu-data-weights")` 重みづけ　集計済みのデータを分析する際などに，変数値に重みをつけるために使用します。
...
分析タブは，統計分析を行う場合に使用します。画面の見た目は変数タブやデータタブと同じですが，リボンの部分が分析用の各ツールボタンになります（[@fig-analyses-tab]）。


```{r fig-analyses-tab, fig.cap="分析タブ", echo=FALSE}
...
:::{.jmvribbon data-latex=""}
+ `r infig("analysis-descriptives")` 探索　平均値などの基本統計量の算出やグラフの作成に使用します（[第[-@sec-ch-exploration]章](#sec-ch-exploration)）
+ `r infig("analysis-ttest")` t検定　1つあるいは2つの平均値の差の検定を行います（[第[-@sec-ch-ttests]章](#sec-ch-ttests)）
+ `r infig("analysis-anova")` 分散分析　3つ以上の平均値の差の検定を行います（[第[-@sec-ch-ANOVA]章](#sec-ch-ANOVA)）
+ `r infig("analysis-regression")` 回帰分析　回帰分析と呼ばれる手法を用いて変数間の関係を分析します（[第[-@sec-ch-regression]章](#sec-ch-regression)）
+ `r infig("analysis-frequencies")` 度数分析　$\chi^{2}$（カイ二乗）検定など，度数・比率の検定を行います（[第[-@sec-ch-frequencies]章](#sec-ch-frequencies)）
+ `r infig("analysis-factor")` 因子分析　因子分析を実施します（[第[-@sec-ch-factor]章](#sec-ch-factor)）
:::

このように，分析タブのツールは分析手法ごとに非常にわかりやすく整理されています。また，これらの基本メニューに加え，画面の一番右には「`r infig("analysis-more")` モジュール」というツールもあります。これはjamoviにさらに高度な機能を追加したい場合に使用します^[現バージョンのjamoviには，さまざまな散布図を作成するための`scatr`という拡張モジュールが標準でインストールされており，このモジュールをオンにすると探索ツールに「Scatterplot（散布図）」と「Pareto Chart（パレート図）」というメニュー項目が追加されます。これらの項目については，本書では省略します。]。

これら分析ツールの使用方法については，「分析編」で詳しく説明します。

#### グラフタブ {-#subsubsec:graph-tab}
...
```{r fig-graph-tab, fig.cap="グラフタブ", echo=FALSE}
knitr::include_graphics("images/basics/graph-tab.png")
```
...
編集タブでは，フォントや段落の設定など，編集に関連したツールボタンがリボンに表示されます（[@fig-edit-tab]）。

```{r fig-edit-tab, fig.cap="編集タブ", echo=FALSE}
...
jamoviが扱う*変数*には，*データ変数*，*計算変数*，*変換変数*の3種類があります^[この他に*結果変数*などもありますが，これらは分析によって自動的に作成されるもので，基本的に編集できません。そのため，ここの説明には含めません。]。このうち，もっとも基本的で重要なものがデータ変数です。ここではまず，データ変数を用いてjamoviにおけるデータの操作方法を見ていきましょう。
...
データファイルからデータを読み込む方法についてはすでに[第[-@sec-ch-introduction]章](#sec-ch-introduction)の「[ファイル操作](#sec-intro-file)」で説明しましたので，ここではデータタブのスプレッドシートを用いたデータの編集方法について見ておきましょう。
...
では，実際にスプレッドシートにデータを入力してみましょう。ここでは[@tbl-basics-datavariable-sampledata]のデータを用いることにします。変数タブが選択されている場合には，データタブを選択してスプレッドシートを表示させてください。

```{r tbl-basics-datavariable-sampledata}
...
入力が終わった状態では，画面は[@fig-data-sample-data]のようになっているはずです。

```{r fig-data-sample-data, fig.cap="データ変数の入力例",echo=F, fig.align="center"}
...
今度は，入力済みデータを行・列の単位で編集する方法についてみてみましょう。jamoviでは，データの行と列はそれぞれ異なった意味をもちますので，追加・削除のメニューは行と列のそれぞれに別々に用意されています（[@fig-edit-row-col]）。

```{r fig-edit-row-col, fig.cap="行・列の編集",echo=F, fig.align="center"}
...
すでに入力されているデータに新たな参加者のデータを追加する場合は単純で，データがまだ入力されていない行にデータを入力するだけです。そうでなく，途中にデータを追加したい場合には，データを挿入したい部分を選択してリボンの「行」にある「追加」から「挿入」を選択します（[@fig-add-row]）。

```{r fig-add-row, fig.cap="行の追加",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/add-row.png")
```

なおこの際，「挿入」の代わりに「追加」を実行すると，データの最後に新しい行が追加されます。

また，不要な行がある場合には，その行を選択し，リボンの「行」の部分にある「削除」を実行します（[@fig-edit-row-delete]）。

```{r fig-edit-row-delete, fig.cap="行の削除",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/edit-row-delete.png")
```

なお，これらの操作は，右クリックで表示されるメニューからも行うことができます（[@fig-add-row-submenu]）。

<!-- ボタンの画面 -->
```{r fig-add-row-submenu, fig.cap="行の追加",echo=F, fig.align="center"}
...
ただし，jamoviが扱う変数には「データ変数」，「計算変数」，「変換変数」の3種類がありますので，変数を追加する際にはどの種類の変数を追加するのかを指定しなければなりません（[@fig-edit-col-add]）。

```{r fig-edit-col-add, fig.cap="列（変数）の追加",echo=F, fig.align="center"}
...
「A」列の変数名の部分（「A」の部分）をダブルクリックするか，または「A」列を選択してからリボンの「`r infig("menu-variable-edit")` 設定」ボタンをクリックしてください（[@fig-data-variable-open]）。


```{r fig-data-variable-open, fig.cap="データ変数の設定パネルを開く",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-variable-open.png")
```

すると，データ変数の設定パネルが表示されますので（[@fig-data-variable]），この画面の「データ変数」の欄に変数名を入力します。データの1列目には「ID」が入力されているので，変数名は「ID」としておきましょう。

```{r fig-data-variable, fig.cap="データ変数の設定パネル",echo=F, fig.align="center"}
...
設定パネルの「測定値型」は，変数の尺度型の設定です。データ変数には，測定値の尺度水準に応じて次の4つの型があります（[@fig-data-variable-measure-type]）。

```{r fig-data-variable-measure-type, fig.cap="データ変数の尺度型",echo=F, fig.align="center"}
...
データ1列目の`ID`変数は個人を特定するための番号ですので，尺度タイプは「`r infig("variable-id")` ID」型にしておきましょう。2列目の「性別」変数は「`r infig("variable-nominal")` 名義」型，「年齢」から「体重」まではすべて「`r infig("variable-continuous")` 連続」型です。
...
ほとんどの場合，データ型についてはjamoviが自動的に判定してくれますので，設定の必要はないでしょう。何からの理由でデータ型を指定したい場合，データ変数の測定型と指定できるデータ型の関係は[@tbl-measure-data-type]のとおりです。

```{r, tbl-measure-data-type, echo=FALSE, message=FALSE}
  data <- data.frame(
    a = c("×", "○", "○"),
    b = c("○", "○", "○"),
    c = c("○", "○", "×")
...
このような場合には，この「水準」の右横にある上下矢印で変数値を適切な順番に並べ替えることで，分析においても「S・A・B・C・D」という順が維持されるようになります（[@fig-data-variable-levels-order]）。

```{r fig-data-variable-levels-order, fig.cap="データ順序の設定",echo=F, fig.align="center"}
...
そのような場合，この「Levels」の「1」の欄に「男」，「2」の欄に「女」と入力し，それぞれの値に*変数ラベル*をつけることができます。そうすると，データ画面上や結果の表示では性別データの「1」が「男」，「2」が「女」と表示されるようになり，男女のとり違えといったミスを防ぐことができるのです（[@fig-data-variable-levels]）。

```{r fig-data-variable-levels, fig.cap="変数ラベルの設定",echo=F, fig.align="center"}
...
さて，これでサンプルデータの入力と設定は完了です。変数の設定パネルを閉じるとスプレッドシートは[@fig-data-sample-data-complete]のようになっているはずです。

```{r fig-data-sample-data-complete, fig.cap="データの入力と設定が終わった後の画面",echo=F, fig.align="center"}
...
計算変数を使用するには，スプレッドシートで列名が空欄になっている部分をダブルクリックするか，その列を選択してリボンの「`r infig("menu-variable-edit")` 設定」ボタンをクリックします。すると，[@fig-data-new-variable]のような変数作成画面が表示されますので，ここで「計算変数を作成」のボタンをクリックします。

```{r fig-data-new-variable, fig.cap="新規に変数を作成する",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-new-variable.png")
```

あるいは，新しい列を選択した状態でリボンの「`r infig("menu-compute")` 計算」ボタンをクリックしても，新しい列に計算変数が作成されて変数設定パネルになります。

計算変数の設定パネルは[@fig-data-computed-variable]のようになっています。変数名と説明の部分はデータ変数の場合と同じです。この計算変数の変数名は「BMI」としておきましょう。

```{r fig-data-computed-variable, fig.cap="計算変数の設定パネル",echo=F, fig.align="center"}
...
このとき，変数名の入力ミスなどがあるとエラーになってしまいますので注意してください。変数名の入力ミスを防ぐには，数式ボックスのすぐ左隣にある「関数ボタン（`r infig("filters-formula")`）」を使うのがよいでしょう。この部分をクリックすると，「関数」と「変数」という画面が表示されます（[@fig-data-computed-variable-function]）。左側の「関数」の部分は，平均値などの計算用の関数のリストで，それぞれの関数名をクリックするとその関数簡単な説明が下側に表示されるようになっています。

```{r fig-data-computed-variable-function, fig.cap="計算変数の作成結果",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-computed-variable-function.png")
```

ここでは関数は使用しませんが，「関数」は，複数の質問項目の回答値を平均して合成得点を求めたりする場合に役立ちます。なお，計算変数に使用できる関数は，次のグループに分類されています。これらの関数の詳細については[付録](#sec-ch-appendix)にまとめてありますのでそちらを参照してください。
...
```{r fig-data-sample-data-computed, fig.cap="関数と変数のリスト",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-sample-data-computed.png")
```

すると，スプレッドシートの「体重」の横に「BMI」という変数が作成され，そこに各個人のBMI値が表示されます（[@fig-data-sample-data-computed]）。なお，この計算変数に格納されているのは式だけですので，計算変数の作成後に計算に使われている変数（「体重」と「身長」）の値が変更されると自動的に再計算されて新しい値に更新されます。計算変数は，スプレッドシートでは変数名の右上部分に黒丸がついた状態で表示されます。
...
スプレッドシートで「身長」の列を選択したら，リボンの「`r infig("menu-data-transform")` 変換」をクリックして変換変数を1つ作成してください。「身長」を選択してから「`r infig("menu-data-transform")` 変換」をクリックすると，変数名などが自動で入力された状態で設定パネルが開きます（[@fig-data-transformed-setting]）。


```{r fig-data-transformed-setting, fig.cap="変換関数の設定パネル",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-transformed-setting.png")
```

変数名は，これがメートル単位の身長だということがわかりやすいように「身長 (m)」としておきましょう。その下の「変換元の変数」の部分は，変換を適用する変数を指定します（[@fig-data-transformed-source-select]）。「身長」の列を選択してから変換変数を作成した場合，ここは自動的にその変数が選択された状態になります。もし変換対象とは異なる変数が指定されている場合には，適切な変数を選択してください。

```{r fig-data-transformed-source-select, fig.cap="変換対象の選択",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-transformed-source-select.png")
```

変換対象を適切に設定したら，その下の「使用する変換」の部分で適用する変換を指定します。今はまだ適用できる変換がありませんので，「変換を新規作成…」を選択して変換を作成しましょう（[@fig-data-transformed-transform-select]）。

```{r fig-data-transformed-transform-select, fig.cap="変換の指定",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-transformed-transform-select.png")
```

すると，[@fig-data-transformed-transform]のような設定パネルが表示されます。

```{r fig-data-transformed-transform, fig.cap="変換の設定パネル",echo=F, fig.align="center"}
...
関数ボタンと数式ボックスの使用方法は計算変数の場合と同じです。必要に応じて関数ボタンで変数名や関数を選択し，数式ボックスの中に変換式を記入します。今回の変換ではセンチメートルで記録された身長のデータをメートルに変換するので，数式ボックス内の式は「\$source / 100」となります（[@fig-data-transformed-transform-complete]）。なお，「\$source」は変換元の値を意味します。

```{r fig-data-transformed-transform-complete, fig.cap="変換式の作成",echo=F, fig.align="center"}
...
```{r fig-data-transformed-transform-recoded, fig.cap="変換の設定が完了した様子",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-transformed-transform-recoded.png")
```

すると，「使用する変換」の部分が先ほど作成した「cm -&gt; m」になっているはずです（[@fig-data-transformed-transform-recoded]）。この変換変数も，計算変数の場合と同様に，変換元の変数値が変化すると自動的に値が更新されます。また，この変換式は，その右横にある「編集…」ボタンをクリックすることで修正可能です。変換変数は，スプレッドシートでは変数名の欄の右上に赤丸がついた状態で表示されます。
...
## 重みづけ {#sec-data-weight}
通常，分析用データは1行に1人分のデータを入力します。しかし，年齢別・性別に集計されたデータのように，すでに集計済みのデータを分析したい場合もあります。このようなデータでそのまま平均値を求めると，各行に含まれる人数が反映されず，正しい値になりません。そこで，各行が何人分の集計なのかを示す*重み*変数を設定すると，集計済みデータも分析用データとして扱えるようになります。

次の例を見てみましょう。このデータには，地区に集計されたある得点が入力されています（[@fig-data-weights-example]）。各地区の回答者数は，それぞれ125人，35人，89人で，3列目の「平均スコア」は，各地区ごとの平均値です。このようなデータでは，回答者全体の「平均スコア」を算出したい場合にそのまま平均値を求めようとすると，単純にこの3つの値の平均値が算出されてしまい，回答者全体の平均値を求めることができません。

```{r fig-data-weights-example, fig.cap="フィルタの設定パネル",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-weights-example.png")
```

回答者の人数を考慮して全体の平均値を求めたい場合には，各行を回答者数で重みづけする必要があります。リボンの「`r infig("menu-data-filter")` 重みづけ」をクリックすると，[@fig-data-weights-setting]のような設定パネルが表示されます。

```{r fig-data-weights-setting, fig.cap="フィルタの設定パネル",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-weights-settings.png")
```

設定パネルの左側は，データに含まれる変数一覧です。ここから「回答者数」を選択し，それをダブルクリック，またはマウスでドラッグして右側の「重みづけの変数」に移動させると，結果ウィンドウに次のように表示され（[@fig-data-weights-result]），以後の分析では回答者数を考慮した値が算出されるようになります。この状態で，「平均スコア」の平均値を求めると，その値は3つの値の平均値ではなく，249人の回答値の平均値になるのです。

```{r fig-data-weights-result, fig.cap="フィルタの設定パネル",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-weights-result.png")
```
...
フィルタを使用するには，リボンの「`r infig("menu-data-filter")` フィルタ」をクリックします。すると，[@fig-data-filter-setting]のような設定パネルが表示されます。

```{r fig-data-filter-setting, fig.cap="フィルタの設定パネル",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-filter-setting.png")
```

設定パネルの左側にあるボタン（`r infig("filters-add")`）はフィルタの追加ボタンです。フィルタは必要に応じて複数設定することができます。その下の目の形をしたアイコン（`r infig("filters-visible")`）は，フィルタにあてはまる行だけを表示させるかどうかを切り替えるボタンです。
...
では，男性のデータだけを表示させるフィルタを作成しましょう。条件式のボックスに「`性別 == 1`」と入力してください。このとき，「性別」と「1」の間の等号（=）は2つ連続で入力する必要がある点に注意してください。式を入力して確定すると，スプレッドシートの表示が[@fig-data-filter-gender]のようになります。

```{r fig-data-filter-gender, fig.cap="男性フィルタの設定後",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/data-filter-gender.png")
```

「フィルタ1」の列に「`r infig("correct")`」が表示されているのがこのフィルタの条件にあてはまるデータ行，「`r infig("incorrect")`」が表示されているのがフィルタにはあてはまらないデータ行です。見てわかるように，性別が「男」の行だけ「`r infig("correct")`」になっています。また，フィルタ設定パネルあるいはスプレッドシート下部にある`r infig("filters-visible")`アイコンをクリックすると，フィルタにあてはまらない行を非表示にすることができます（[@fig-data-filter-on-off]）。

```{r fig-data-filter-on-off, fig.cap="フィルタ表示方法の変更",echo=F, fig.align="center"}
...
ここまでの作業で作成した変数やフィルタは，「変数」タブで一覧することもできます。[@fig-data-variables-tab]は，ここまでに作成した変数やフィルタに「説明」をいくつか追加した状態です。このように，変数タブでは変数名と変数の説明がウィンドウに一覧表示されるので，変数名を英数字にして「説明」欄に日本語で説明を記入した場合や，質問紙調査データで「説明」欄に質問文を入力した場合などには，このタブは重宝することでしょう。

```{r fig-data-variables-tab, fig.cap="変数タブでの変数一覧",echo=F, fig.align="center"}
...
```{r fig-basics-exercise, fig.cap="練習用データファイル",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise.png")
```

このデータファイルには，「ID」，「年齢」，「性別」の各変数値と，「Q1」から「Q7」までの回答値が入力されています（[@fig-basics-exercise]）。「Q1」から「Q7」は，それぞれ「外向性」を測定する心理尺度の回答データで，いずれも「まったくそう思わない（1）」から「とてもそう思う（7）」までの7段階で回答されています。「Q1」から「Q7」の質問内容は，変数タブで確認することができます（[@fig-basics-exercise-variables]）^[この尺度は，この練習用に作成した架空のものです。]。

```{r fig-basics-exercise-variables, fig.cap="サンプルデータの変数一覧",echo=F, fig.align="center"}
...
この際，それらの質問文の中には「よく知らない人との交流は苦手だ」のような，外向性の高さとは逆の内容の文が含まれていることがあります。このように，本来測定したいものとは逆の内容を含む質問文のことを*逆転項目*といいます（[@fig-basics-exercise-reverse]）。

```{r fig-basics-exercise-reverse, fig.cap="逆転項目",echo=F, fig.align="center"}
...
まず，「Q2」の列名のところをクリックして，「Q2」の列を選択します（[@fig-basics-exercise-select-col]）。

```{r fig-basics-exercise-select-col, fig.cap="列の選択",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise-select-col.png")
```


その状態でデータタブの「変数」にある「`r infig("menu-data-transform")`  変換」ボタンをクリックします（[@fig-basics-exercise-add-transform]）。

```{r fig-basics-exercise-add-transform, fig.cap="変換変数の追加",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise-add-transform.png")
```


すると，[@fig-basics-exercise-set-transform]のような変換変数の設定パネルが表示されますので，作成した変換変数の名前を「Q2.R」に設定しましょう。このとき，「変換元の変数」の部分が「Q2」になっていることを確認してください。

```{r fig-basics-exercise-set-transform, fig.cap="変換変数名の設定",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise-set-transform.png")
```

次に，「使用する変換」のメニューから「変換を新規作成…」を選択して実行します（[@fig-basics-exercise-using-transform]）。

```{r fig-basics-exercise-using-transform, fig.cap="「変換」の作成",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise-using-transform.png")
```

すると，[@fig-basics-exercise-transform-name]のような「変換」の設定パネルが表示されます。再利用の際にわかりやすいように，この変換の名前を「逆転項目」にしましょう。

```{r fig-basics-exercise-transform-name, fig.cap="変換名の設定",echo=F, fig.align="center"}
...
そこで，変換式の部分を次のように設定します（[@fig-basics-exercise-transform-formula]）。

:::{.jmvformula data-latex=""}
= 8 - $source
:::

```{r fig-basics-exercise-transform-formula, fig.cap="変換式の設定",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise-transform-formula.png")
```


画面右側の`r infig("action-back-circle-d")`をクリックして変換の設定パネルを閉じ，`r infig("action-back-circle-u")`で変数設定パネルを閉じると，「Q2」のすぐ後ろの列に「Q2.R」という変換変数が作成されていることが確認できます
（[@fig-basics-exercise-trans-q2]）。この変数が変換変数であることは，変数名の部分の右肩に小さな赤丸がついていることから判断できます。

この変数の値を確認すると，「Q2」の値が「1」の場合には「Q2.R」は「7」，「Q2」の値が「7」の場合には「Q2.R」は「1」というように，値が逆転しているのがわかると思います。

```{r fig-basics-exercise-trans-q2, fig.cap="作成した変換変数とその目印",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise-trans-q2.png")
```

`Q2`の逆転処理はこれでおしまいです。次は「Q4」の逆転処理を行いましょう。「Q4」の列を選択して「`r infig("menu-data-transform")` 変換」をクリックすると，再び変換変数の設定パネルが表示されます。新しく作成した変換変数の名前を「Q4.R」に設定したら，「変換元の変数」の部分が「Q4」になっていることを確認し，「使用する変換」のメニューをクリックしてください。すると，メニュー項目の中に，先ほど作成した「逆転項目」があるはずです
（[@fig-basics-exercise-set-transform-q4]）。

```{r fig-basics-exercise-set-transform-q4, fig.cap="作成済みの変換を他の変数に適用",echo=F, fig.align="center"}
...
（[@fig-basics-exercise-add-compvar]）。

```{r fig-basics-exercise-add-compvar, fig.cap="計算変数の追加",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise-add-compvar.png")
```


作成された計算変数の変数名部分をダブルクリックすると，変数の設定パネルが表示されます。変数名は「N」になっているかと思いますが，このままだと何の値か分からないので，変数名を「外向性」に変更します
（[@fig-basics-exercise-gaiko]）。

```{r fig-basics-exercise-gaiko, fig.cap="計算変数の名前を設定",echo=F, fig.align="center"}
...
（[@fig-basics-exercise-sum]）。この「SUM」という関数は，括弧の中に指定された値の合計値を求める関数です^[尺度得点に平均値を用いる場合には，「MEAN」関数を使用します。これらの関数の詳細については付録の[関数一覧](#sec-ch-appendix)を参照してください。]。

```{r fig-basics-exercise-sum, fig.cap="関数一覧からSUM関数を入力する",echo=F, fig.align="center"}
...
（[@fig-basics-exercise-sum-complete]）。

:::{.jmvformula data-latex=""}
= SUM(Q1, Q2.R, Q3, Q4.R, Q5, Q6.R, Q7)
:::

```{r fig-basics-exercise-sum-complete, fig.cap="SUM関数を完成させる",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise-sum-complete.png")
```


式の入力が終わったら，画面右の`r infig("action-back-circle-u")`をクリックして変数設定パネルを閉じます。これで各対象者の外向性得点（合計値）が算出されました
（[@fig-basics-exercise-sumscore]）。

```{r fig-basics-exercise-sumscore, fig.cap="算出された外向性得点",echo=F, fig.align="center"}
...
（[@fig-basics-exercise-grouping]）。

```{r fig-basics-exercise-grouping, fig.cap="グループ分け用の計算変数を追加",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise-grouping.png")
```


そして新しく作成した計算変数の設定パネルを開き，変数名を「群」に変更しましょう
（[@fig-basics-exercise-group-name]）。

```{r fig-basics-exercise-group-name, fig.cap="計算変数の名前を`群`に設定",echo=F, fig.align="center"}
...
（[@fig-basics-exercise-group-if]）。このIF関数は，条件式の結果に応じて異なる値を設定できる関数です。

```{r fig-basics-exercise-group-if, fig.cap="計算式でIF関数を使用",echo=F, fig.align="center"}
...
（[@fig-basics-exercise-group-if-formula]）。

```{r fig-basics-exercise-group-if-formula, fig.cap="条件式の入力",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise-group-if-formula.png")
```

さらに，関数一覧から「VMEAN」を選択して入力します
（[@fig-basics-exercise-group-vmean]）。この「VMEAN」は，指定した変数の平均値を算出する関数です^[平均値を算出する関数には「MEAN」と「VMEAN」の2つがありますが，「MEAN」は複数の変数値の平均値を「各対象者ごと」に計算するのに対し，「VMEAN」は1つの変数値について「対象者全体」での平均値を算出する点が異なります。]。

```{r fig-basics-exercise-group-vmean, fig.cap="全体の平均値の算出式を入力",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise-group-vmean.png")
```


そしてこの「VMEAN」関数の括弧の中に「外向性」を指定します
（[@fig-basics-exercise-group-vmean-gaiko]）。

```{r fig-basics-exercise-group-vmean-gaiko, fig.cap="平均値を算出する変数を指定",echo=F, fig.align="center"}
...
（[@fig-basics-exercise-group-if-full]）。このとき，変数値に「高群」や「低群」のような文字を用いる場合には，その値を引用符（""）で括る必要がありますので注意してください。
...
```{r fig-basics-exercise-group-if-full, fig.cap="グループ分けの式を完成させる",echo=F, fig.align="center"}
knitr::include_graphics("images/basics/exercise-group-if-full.png")
```

式を完成させて変数設定パネルを閉じると，新しく作成した「群」変数に「高群」または「低群」が値として入力されているのを確認することができます
（[@fig-basics-exercise-group-var]）。条件式の部分がやや難しく感じられたかもしれませんが，計算変数ではこのようにしてデータをグループ分けすることもできるのです。

```{r fig-basics-exercise-group-var, fig.cap="グループ分けの変数の完成",echo=F, fig.align="center"}
