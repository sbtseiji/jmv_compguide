...
2つ目（図\@ref(fig:regression-binomial-results)の右側）の表は回帰係数に関する結果です。この部分は線形回帰分析の結果と基本的に同じで，「**Estimate**（推定値）」の部分が回帰係数，「**SE**（標準誤差）」が回帰係数の標準誤差です。ただし，2項ロジスティック回帰では，回帰係数の有意性についての検定にはtではなく「**Z**」が用いられます。そして，Zの隣の「**p**」の値が有意水準を下回っている場合に，回帰係数が有意である（0でない）と判断します。
