...
out<-jmvReadWrite::jmvWrite(data01,'data/omv/basics2_data01.omv')
...
```{r bs2-analyses-tab, fig.cap='Analyses（分析）タブ', echo=FALSE}
knitr::include_graphics("images/basics2/analyses-tab.png")
```
...
jamoviでの記述統計量の算出には，Analysesタブの「`r infig('analysis-descriptives')` Exploration（探索）」を使用します。「Exploration」ツールボタンをクリックすると「Descriptives（記述統計）」という項目が表示されるので，それを選択してください（図\@ref(fig:bs2-descriptives-menu)）。
...
そして，ここで分析対象の変数を「年齢」から「得点」に変更すると，分析結果も「得点」の記述統計量に変更されるのです。これなら，古い分析結果と新しい分析結果が混在してごちゃごちゃになるということがありません。分析対象の変数を「年齢」から「得点」に変更するには，「Variables」にある「年齢」を選択して「`r infig('action-back')`」で変数一覧に戻した後に，「得点」を「Variables」に入れるだけです。すると，すぐに結果の表の中身が「得点」のものに更新されます（図\@ref(fig:bs2-change-var)）。
...
「`r infig('analysis-ttest')`  T-Tests（t検定）」は，1つあるいは2つの平均値の検定のためのツールです。jamoviの基本構成では，ここには次の3つの分析メニューが含まれています（図\@ref(fig:ttests-ttest-menu)）。

```{r ttests-ttest-menu, fig.cap='T-Tests（t検定）のメニュー', echo=FALSE}
...
out<-jmvReadWrite::jmvWrite(data01,'data/omv/ttest_data01.omv')
...
それでは，グループ1とグループ2で，課題得点の平均値に差があるといえるかどうかを検定してみましょう。対応なしt検定を行うには，Analysesタブの「`r infig('analysis-ttest')` T-Tests」から「Independent Samples T-Test（対応なしt検定）」を選択します（図\@ref(fig:ttests-independent-menu)）。
...
<!-- -->
```{r ttests-tests, fig.cap='Test（検定）の設定項目', echo=FALSE}
...
```{r ttests-hypothesis, fig.cap='Hypothesis（仮説）の設定項目', echo=FALSE}
...
<!-- -->
```{r ttests-missing-values, fig.cap='Missing values（欠損値）の設定項目', echo=FALSE}
...
<!-- -->
```{r ttests-additional, fig.cap='Additional Statistics（追加の統計量）の設定項目', echo=FALSE}
...
out<-jmvReadWrite::jmvWrite(data02,'data/omv/ttest_data02.omv')
...
それでは，訓練前と訓練後で，記憶課題成績の平均値に差があるといえるかどうかを検定してみましょう。対応ありt検定を行うには，Analysesタブの「`r infig('analysis-ttest')`  T-Tests（t検定） 」から「Paired Samples T-Test（対応ありt検定）」を選択します（図\@ref(fig:ttests-paired-t-menu)）。
...
このとき，「訓練前」と「訓練後」を1つずつ「Paired Variables」欄に移動することもできますが，2つまとめて移動させる方がわかりやすいでしょう。まずどちらか一方の変数名をクリックして選択した後，「SHIFT(⇧)」キーを押しながらもう一方の変数名をクリックすると2つの変数を同時に選択することができますので，その状態で「`r infig('action-forward')`」をクリックして変数ペアの設定を行います（図\@ref(fig:ttests-paired-variables)）。
...
```{r ttests-paired-test, fig.cap='Test（検定）の設定項目', echo=FALSE}
...
out<-jmvReadWrite::jmvWrite(data03,'data/omv/ttest_data03.omv')
...
1標本t検定を行うには，Analysesタブの「`r infig('analysis-ttest')`  T-Tests（t検定）」から「One T-Test（1標本t検定）」を選択します（図\@ref(fig:ttests-one-menu)）。
...
データファイルを開いたら，「変化量」という名前の計算変数を作成し，そこに記憶訓練実施前の得点（「訓練前」）と実施後の得点（「訓練後」）の差を格納しましょう。まず，Dataタブの「Variables」にある「`r infig('menu-variable-add')` Add（変数の追加）」ボタンから「Computed Variable（計算変数）」の「`r infig('menu-computed-append')` Append」を選択して新たに計算変数を追加します（図\@ref(fig:ttests-append-compvar)）。
...
```{r ttests-compvar-diff, fig.cap='作成したDIFF変数', echo=FALSE}
knitr::include_graphics("images/ttests/compvar-diff.png")
```


変数の作成が完了したら，Analysesタブの「`r infig('analysis-ttest')` T-Tests」から「One Sample T-Test」を選択します（図\@ref(fig:ttests-diff-one)
）。
