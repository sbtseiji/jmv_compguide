...
\hypertarget{preface}{%
\chapter*{はじめに}\label{preface}}
\addcontentsline{toc}{chapter}{はじめに}

\IntroChapter{はじめに}{Intro}

\jlogo{}は，高度な統計手法をできるだけ簡単に使用できるようにすることを目的として作成された統計ソフトウェアです。操作はマウス（トラックパッド）によるメニュー選択が基本で，複雑なコマンドを入力したりする必要はありません。jamoviと同じようにマウスで操作できる統計ソフトウェアとして非常に有名なものに\href{https://www.ibm.com/jp-ja/analytics/spss-statistics-software}{SPSS}があり，大学を含め，さまざまな機関で統計処理や統計法の学習に使用されていますが，SPSSは非常に高価なソフトウェアであり，個人で気軽に購入できるようなものではないため，大学のコンピューター室でしか課題作業ができない（家で課題をできない）というような問題がありました。

しかし，近年ではオープンソース\footnote{プログラムの中身が公開されていて，誰でも自由に使えるソフトウェアのことです。}の統計解析環境である\href{https://www.r-project.org}{R}が専門家の間で広く使われるようになっています。オープンソースであるRは基本的に無料で使用できるため，誰もが自由に自分のPCにインストールして統計処理を行えるのです。ただし，Rはコマンドを入力して実行するという形式で分析を進めていくため，使いこなすにはコンピューター・プログラミングの基本的な知識が必要です。そのため，無料で使用できるとはいえ，入門者にはかなりハードルの高いものでした。
...
\addcontentsline{toc}{subsubsection}{Windowsでのインストール方法}
...
\addcontentsline{toc}{subsubsection}{Macでのインストール方法}
...
\addcontentsline{toc}{subsubsection}{LinuxまたはChromebookでのインストール方法}

最近のバージョンでは，LinuxやChromebook\footnote{Chromebookにインストールするには「Linux」サポートが有効になっている必要があります。}でもjamoviを利用できるようになりました。Linuxの場合にはディストリビューションごとに設定方法が少しずつ異なりますので，ここでは具体的なインストール方法は省略します。ウェブサイトに示された手順にしたがってインストールしてください。

\hypertarget{sec:intro-launch-quit}{%
\section{Starting Up：起動と終了}\label{sec:intro-launch-quit}}

まずはソフトウェアの起動と終了の仕方を見ておきましょう。

\hypertarget{sub:intro-launch}{%
\subsection{jamoviの起動}\label{sub:intro-launch}}
...
\caption{jamoviのアプリケーション画面}\label{fig:jamovi-screen}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{Syntax mode：シンタックスモード}
...
\addcontentsline{toc}{subsubsection}{New：新規}
...
\addcontentsline{toc}{subsubsection}{Open：開く}
...
\addcontentsline{toc}{subsubsection}{Import：読み込み}
...
\hypertarget{sec:intro-file-save}{%
\subsubsection*{Save, Save As：保存，別名で保存}\label{sec:intro-file-save}}
\addcontentsline{toc}{subsubsection}{Save, Save As：保存，別名で保存}

入力したデータや分析結果をjamoviファイルとして保存するには「\textbf{Save}」または「\textbf{Save As}」を使用します。保存場所を指定するには「\infig{place-browse.pdf} Browse」をクリックしてください。なお，jamoviファイルの拡張子は「.omv」です。

\hypertarget{sec:intro-file-export}{%
\subsubsection*{Export：書き出し}\label{sec:intro-file-export}}
\addcontentsline{toc}{subsubsection}{Export：書き出し}
...
\item
  ...
  \item
    Webページ（.html，.htm）
  \item
    LaTeXバンドル\footnote{LaTeXはオープンソースの組版システムです。}（.zip）
  \end{itemize}
\item
  分析手順をテンプレートとして書き出す
...
\hypertarget{sec:intro-exercise1}{%
\section{Exercise：実践 ファイルを開く}\label{sec:intro-exercise1}}

\hypertarget{jamovidatafilenobaai}{%
\subsubsection*{jamoviデータファイルの場合}\label{jamovidatafilenobaai}}
\addcontentsline{toc}{subsubsection}{jamoviデータファイルの場合}
...
\tightlist
\item
  練習用データファイル1（\href{https://github.com/sbtseiji/jmv_compguide/raw/main/data/omv/intro_data01.omv}{intro\_data01.omv}）
\end{itemize}

このファイルはjamoviのデータファイルなので，通常はこのファイルをダブルクリックすればjamoviで開くことができるのですが，ここではファイルメニューを使ってこのファイルを開いてみます。

\hypertarget{iti}{%
\paragraph*{手順1}\label{iti}}
\addcontentsline{toc}{paragraph}{手順1}

「≡」をクリックしてファイルメニューを開きます。

\hypertarget{ni}{%
\paragraph*{手順2}\label{ni}}
\addcontentsline{toc}{paragraph}{手順2}

「Open」で「This PC」が選択されていることを確認します。

\hypertarget{san}{%
\paragraph*{手順3}\label{san}}
\addcontentsline{toc}{paragraph}{手順3}
...
\addcontentsline{toc}{paragraph}{手順4}
...
\addcontentsline{toc}{subsubsection}{テキストファイル・Excelファイルの場合}
...
\addcontentsline{toc}{subsubsection}{Data：データタブ}
...
\caption{データタブのツールボタン}\label{fig:data-overview}
\end{figure}
...
\tightlist
\item
  \textbf{Clipboard}（クリップボード）　データのコピー（\infig{menu-data-copy.pdf}）や貼り付け（\infig{menu-data-paste.pdf}），切り取り（\infig{menu-data-cut.pdf}）などに使用します。使い方はWordやExcelなどと基本的に同じです。
\item
  \textbf{Edit}（編集）　操作の取り消し（\infig{menu-undo.pdf}），やり直し（\infig{menu-redo.pdf}）を行います。
\item
  \textbf{Variables}（変数）　変数の設定や変数値の処理に使用するツール群です。
  ...
  \end{itemize}
\item
  \textbf{Rows}（データ行）　データ行の追加や削除に使用するツール群です。
...
変数操作に関するボタン群，データ行の編集に関するボタン群の詳しい使い方については「\protect\hyperlink{sec:data-handling}{データ変数}」の操作のところで説明します。

\hypertarget{subsubsec:analyses-tab}{%
\subsubsection*{Analyses：分析タブ}\label{subsubsec:analyses-tab}}
\addcontentsline{toc}{subsubsection}{Analyses：分析タブ}
...
\caption{Analyses（分析）タブのツールボタン}\label{fig:analyses-tab}
\end{figure}
...
このように，分析タブのツールは分析手法ごとに非常にわかりやすく整理されています。また，これらの基本メニューに加え，画面の一番右には「\infig{analysis-more.pdf}Modules（モジュール）」というツールもあります。これはjamoviにさらに高度な機能を追加したい場合に使用します\footnote{jamovi 1.8以降のバージョンでは，さまざまな散布図を作成するための\texttt{scatr}というモジュールが標準でインストールされており，このモジュールをオンにするとExplorationツールに「Scatterplot(散布図)」と「Pareto Chart（パレート図）」いうメニュー項目が追加されます。これらの項目については，本書では省略します。}。

これら分析ツールの使用方法については，「分析編」で詳しく説明します。

\hypertarget{subsubsec:edit-tab}{%
\subsubsection*{Edit：編集タブ}\label{subsubsec:edit-tab}}
\addcontentsline{toc}{subsubsection}{Edit：編集タブ}
...
\caption{Edit（編集）タブのツールボタン}\label{fig:edit-tab}
\end{figure}
...
\item
  Edit（編集）　\textbf{Data}タブのボタンと同じです。
\item
  Font（書体）　文字を太字にしたり，斜体にしたりする際に使用します。
\item
  Paragraph（段落）　文字全体を中央揃えにしたり，箇条書きにしたりなど，段落の書式を変更する際に使用します。
\item
  Insert（挿入）　メモに数式を含めたい場合に使用します。数式はTeX形式で記述します\footnote{厳密には\href{https://katex.org/docs/supported.html}{KaTeX}というシステムの書式を使用しています。}。
\item
...
\addcontentsline{toc}{subsubsection}{行の編集}
...
\addcontentsline{toc}{subsubsection}{列の編集}
...
\hypertarget{subsubsec:data-data-setup}{%
\subsubsection*{データ変数の設定}\label{subsubsec:data-data-setup}}
\addcontentsline{toc}{subsubsection}{データ変数の設定}
...
\caption{データ変数の設定画面を開く}\label{fig:data-variable-open}
\end{figure}
...
\caption{データ変数の設定画面}\label{fig:data-variable}
\end{figure}
...
\hypertarget{measure-typeux5c3aux5ea6ux578b}{%
\paragraph*{Measure type（尺度型）}\label{measure-typeux5c3aux5ea6ux578b}}
\addcontentsline{toc}{paragraph}{Measure type（尺度型）}
...
\caption{データ変数の尺度型}\label{fig:data-variable-measure-type}
\end{figure}
...
\tightlist
\item
  \infig{variable-nominal.pdf} Nominal（名義）型　\emph{名義尺度}で測定されたデータです。
\item
  \infig{variable-ordinal.pdf} Ordinal（順序）型　\emph{順序尺度}で測定されたデータです。
\item
  \infig{variable-continuous.pdf} Continuous（連続）型　\emph{比率尺度}や\emph{間隔尺度}で測定されたデータです。
\item
...
データ1列目の\texttt{ID}変数は個人を特定するための番号ですので，尺度タイプは「\infig{variable-continuous.pdf} ID」型にしておきましょう。2列目の「性別」変数は「\infig{variable-nominal.pdf} Nominal」型，「年齢」から「体重」まではすべて「\infig{variable-continuous.pdf} Continuous」型です。

\hypertarget{data-typeux30c7ux30fcux30bfux578b}{%
\paragraph*{Data type：データ型}\label{data-typeux30c7ux30fcux30bfux578b}}
\addcontentsline{toc}{paragraph}{Data type：データ型}
...
\caption{\label{tab:unnamed-chunk-4}データ変数の測定型とデータ型 \label{tab:measure-data-type}}
\centering
...
\hypertarget{Missinmissin}{%
\paragraph*{Missing values：欠損値}\label{Missinmissin}}
\addcontentsline{toc}{paragraph}{Missing values：欠損値}
...
\hypertarget{levelslvels}{%
\subsubsection*{Levels：水準}\label{levelslvels}}
\addcontentsline{toc}{subsubsection}{Levels：水準}
...
\caption{データ順序の設定}\label{fig:data-variable-levels-order}
\end{figure}
...
\hypertarget{sec:basics-computed-variable}{%
\section{Computed Variable：計算変数}\label{sec:basics-computed-variable}}
...
\hypertarget{sub:basics-computed-variable-new}{%
\subsection{計算変数の作成}\label{sub:basics-computed-variable-new}}
...
\caption{新規に変数を作成する}\label{fig:data-new-variable}
\end{figure}
...
\caption{計算変数の設定画面}\label{fig:data-computed-variable}
\end{figure}

その下の部分はデータ変数の場合とはだいぶ違いますね。計算変数は，この設定画面の中央，数式ボックスの「=」の後ろに計算式を入力して値を設定します。

\hypertarget{sub:basics-computed-variable-setting}{%
\subsection{計算変数の設定}\label{sub:basics-computed-variable-setting}}
...
\caption{計算変数の作成結果}\label{fig:data-computed-variable-function}
\end{figure}
...
\hypertarget{subsec:tranformed-variable}{%
\section{Transformed Variable：変換変数}\label{subsec:tranformed-variable}}

質問紙調査のデータで\emph{逆転項目}が複数ある場合，その回答値の逆転処理は計算変数でも可能ですが，逆転項目がたくさんある場合，その1つ1つに対して計算式を入力して計算変数を作成するのは面倒ですし，途中でミスが発生する可能性も増大します。
...
\caption{変換関数の設定画面}\label{fig:data-transformed-setting}
\end{figure}
...
\caption{フィルタの設定画面}\label{fig:data-filter-setting}
\end{figure}
...
では，男性のデータだけを表示させるフィルタを作成しましょう。条件式のボックスに「\texttt{性別\ ==\ 1}」と入力してください。このとき，「性別」と「1」の間の等号（=）は2つ連続で入力する必要がある点に注意してください。式を入力して確定すると，スプレッドシートの表示が図\ref{fig:data-filter-gender}のようになります。
...
\caption{男性フィルタの設定後}\label{fig:data-filter-gender}
\end{figure}
...
\caption{フィルタ表示方法の変更}\label{fig:data-filter-on-off}
\end{figure}
...
\hypertarget{ux5909ux63dbux5909ux6570ux306eux4f5cux6210}{%
\subsubsection*{「変換変数」の作成}\label{ux5909ux63dbux5909ux6570ux306eux4f5cux6210}}
\addcontentsline{toc}{subsubsection}{「変換変数」の作成}
...
\caption{列の選択}\label{fig:basics-exercise-select-col}
\end{figure}
...
\caption{変換変数の追加}\label{fig:basics-exercise-add-transform}
\end{figure}
...
\caption{変換式の設定}\label{fig:basics-exercise-transform-formula}
\end{figure}
...
\caption{作成した変換変数とその目印}\label{fig:basics-exercise-trans-q2}
\end{figure}

\texttt{Q2}の逆転処理はこれでおしまいです。次は「Q4」の逆転処理を行いましょう。「Q4」の列を選択して「\infig{menu-data-transform.pdf} Transform」をクリックすると，再び変換変数の設定画面が表示されます。新しく作成した変換変数の名前を「Q4.R」に設定したら，「Source variable」の部分が「Q4」になっていることを確認し，「using transform」のメニューをクリックしてください。すると，メニュー項目の中に，先ほど作成した「逆転項目」があるはずです
...
\caption{作成済みの変換を他の変数に適用}\label{fig:basics-exercise-set-transform-q4}
\end{figure}
...
\hypertarget{sub:basics2-scale-score}{%
\subsection{尺度得点の算出}\label{sub:basics2-scale-score}}

「外向性」などの単一の概念について複数の質問文を用いて測定する場合には，それら複数の質問文に対する回答値を合計または平均し，それを\emph{尺度得点}として用いるのが一般的です。ここでは，「Q1」から「Q7」までの回答の合計値を尺度得点として算出することにしましょう。なお，その際，「Q2」や「Q4」などの逆転項目については逆転処理を行った後の変数値（「Q2.R」や「Q4.R」など）を用いる必要がある点に注意してください。
...
\caption{計算変数の追加}\label{fig:basics-exercise-add-compvar}
\end{figure}
...
\caption{計算変数の名前を設定}\label{fig:basics-exercise-gaiko}
\end{figure}
...
（図\ref{fig:basics-exercise-sum}）。この「SUM」という関数は，括弧の中に指定された値の合計値を求める関数です\footnote{尺度得点に平均値を用いる場合には，「MEAN」関数を使用します。これらの関数の詳細については，付録を参照してください。}。
...
\caption{関数一覧からSUM関数を入力する}\label{fig:basics-exercise-sum}
\end{figure}

数式ボックスの中に「\texttt{=\ SUM()}」と入力されているのを確認したら，括弧の間に合計対象となる変数を記入していきます。この際，変数名はキーボードから入力してもよいですが，関数の右側に表示される変数一覧から必要な変数名をダブルクリックして入力したほうが，タイプミスの心配もなく，わかりやすいでしょう。今回，合計値の対象となるのは，「Q1」，「Q2.R」，「Q3」，「Q4.R」，「Q5」，「Q6.R」，「Q7」の7つの変数です。各変数の区切りにはコンマ（,）を使用します。変数名を1つ入力したら，日本語入力はオフにした状態でコンマ（,）を入力し，次の変数名を入力するようにしてください。
...
\caption{SUM関数を完成させる}\label{fig:basics-exercise-sum-complete}
\end{figure}
...
\caption{算出された外向性得点}\label{fig:basics-exercise-sumscore}
\end{figure}

\hypertarget{sub:basics2-split-groups}{%
\subsection{グループへの分割}\label{sub:basics2-split-groups}}
...
\caption{グループ分け用の計算変数を追加}\label{fig:basics-exercise-grouping}
\end{figure}
...
\caption{計算変数の名前を`Group`に設定}\label{fig:basics-exercise-group-name}
\end{figure}
...
\caption{計算式でIF関数を使用}\label{fig:basics-exercise-group-if}
\end{figure}
...
\caption{記述統計量の算出}\label{fig:bs2-descriptives-menu}
\end{figure}
...
\caption{記述統計の設定画面}\label{fig:bs2-descriptives-setting}
\end{figure}
...
\caption{分析対象の指定}\label{fig:bs2-descriptives-var}
\end{figure}
...
\caption{分析結果}\label{fig:bs2-descriptives-res}
\end{figure}
...
\hypertarget{subsec:basic2-modify-analysis}{%
\subsection{分析の修正}\label{subsec:basic2-modify-analysis}}
...
\addcontentsline{toc}{subsubsection}{データ修正後の再分析}
...
\hypertarget{ux5206ux6790ux8a2dux5b9aux306eux5909ux66f4}{%
\subsubsection*{分析設定の変更}\label{ux5206ux6790ux8a2dux5b9aux306eux5909ux66f4}}
\addcontentsline{toc}{subsubsection}{分析設定の変更}
...
\caption{分析設定の変更}\label{fig:bs2-modify-analysis}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{分析の複製}
...
\addcontentsline{toc}{subsubsection}{分析の削除}
...
\addcontentsline{toc}{subsubsection}{注釈の追加}
...
\addcontentsline{toc}{subsubsection}{分析結果の書き出し}
...
\caption{結果のタイトルや説明の編集}\label{fig:bs2-report}
\end{figure}

\hypertarget{part-two}{%
\part{分析編}\label{part-two}}

\hypertarget{ch:exploration}{%
\chapter{Exploration：探索}\label{ch:exploration}}
...
この分析ツールの使い方を見るために，簡単なサンプルデータ（\href{https://github.com/sbtseiji/jmv_compguide/raw/main/data/omv/exploration_data01.omv}{exploration\_data01.omv}）を用いることにします（図\ref{fig:exploration-sample}）。
...
\caption{サンプルデータ}\label{fig:exploration-sample}
\end{figure}

このサンプルデータには，男女計50人分の次の変数値が格納されています\footnote{本書で利用するサンプルデータはすべて架空のデータです。}。
...
\tightlist
\item
  \texttt{ID}　対象者のID：\infig{variable-id.pdf} ID型
\item
  \texttt{性別}　対象者の性別：\infig{variable-nominal.pdf} Nominal（名義）型
\item
  \texttt{年齢}　対象者の年齢：\infig{variable-continuous.pdf} Continuous（連続）型
\item
  \texttt{身長}　対象者の身長（cm）：\infig{variable-continuous.pdf} Continuous（連続）型
\item
  \texttt{体重}　対象者の体重（kg）：\infig{variable-continuous.pdf} Continuous（連続）型
\item
  \texttt{血液型}　対象者の血液型：\infig{variable-nominal.pdf} Nominal（名義）型
\item
  \texttt{体力}　対象者の体力レベル（5段階判定）：\infig{variable-ordinal.pdf} Ordinal（順序）型
\item
  \texttt{学力}　対象者の学力レベル（5段階判定）：\infig{variable-ordinal.pdf} Ordinal（順序）型
\end{itemize}
...
\caption{Exploration（探索）ツールの構成}\label{fig:Exploration-menu}
\end{figure}

それでは，メニューから「Descriptives」を選択してください。これで分析開始です。

\hypertarget{sec:exp-exploration-descriptives}{%
\section{Descriptives：記述統計}\label{sec:exp-exploration-descriptives}}
...
\caption{Descriptives（記述統計）の分析設定画面}\label{fig:Exploration-descriptives-panel}
\end{figure}
...
\caption{「性別」の記述統計量の算出}\label{fig:Exploration-descriptives-seibetsu}
\end{figure}
...
\caption{「性別」の記述統計量}\label{fig:Exploration-descriptives-seibetsu-table}
\end{figure}
...
\caption{度数分布表の表示設定}\label{fig:Exploration-descriptives-seibetsu-frequency-tables}
\end{figure}
...
\caption{度数分布表}\label{fig:Exploration-descriptives-seibetsu-table2}
\end{figure}

\hypertarget{subsec:exp-statistics}{%
\subsection{Statistics：統計量}\label{subsec:exp-statistics}}
...
\caption{Statistics（統計量）の詳細設定}\label{fig:Exploration-descriptives-statistics}
\end{figure}
...
「血液型」でも，性別のときと同様に度数分布表を作成することにしましょう。また，性別と違って血液型には4種類の値が含まれていますので，今回は\emph{最頻値}についても算出することにします。この場合，分析の設定画面は図\ref{fig:Exploration-descriptives-ketsueki}のようになります。
...
\caption{血液型の記述統計}\label{fig:Exploration-descriptives-ketsueki}
\end{figure}

最頻値（Mode）は\texttt{1.00}となっていますが，このデータでは血液型が「A=1，B=2，O=3，AB=4」として入力されているので，最頻値は「A型」ということになります。度数分布表では値そのものではなく変数ラベルが表示されるのですが，記述統計量の表では残念ながら変数ラベルではなく値そのものが表示されるので，その点は注意が必要です。

\hypertarget{subsub:exp-ordinal}{%
\subsubsection*{順序尺度データの記述統計}\label{subsub:exp-ordinal}}
\addcontentsline{toc}{subsubsection}{順序尺度データの記述統計}
...
\hypertarget{subsub:exp-continuous}{%
\subsubsection*{間隔・比率尺度データの記述統計}\label{subsub:exp-continuous}}
\addcontentsline{toc}{subsubsection}{間隔・比率尺度データの記述統計}
...
「年齢」，「身長」，「体重」の3つを「Variables」に移動したら，せっかくなので［Statistics（統計量）］で設定できるさまざまな値を算出してみましょう\footnote{ここでは説明のためにいろいろな値を算出していますが，実際の分析では必要なものだけを選んで実行してください。}。「\textbf{Dispersion}（ばらつき）」，「\textbf{Mean Dispersion}（平均値のばらつき）」，「\textbf{Distribution}（分布）」，「\textbf{Normality}（正規性）」にある項目のすべてにチェックを入れてみてください（図\ref{fig:Exploration-descriptives-continuous-setting}）。
...
\caption{分析の設定}\label{fig:Exploration-descriptives-continuous-setting}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{グループごとの記述統計}
...
\hypertarget{subsec:exp-plots}{%
\subsection{Plots：作図}\label{subsec:exp-plots}}
...
\hypertarget{subsub:exp-plot-nominal}{%
\subsubsection*{名義型・順序型データの視覚化}\label{subsub:exp-plot-nominal}}
\addcontentsline{toc}{subsubsection}{名義型・順序型データの視覚化}
...
また，第\ref{ch:basics-more}章の「\protect\hyperlink{subsec:basic2-modify-analysis}{分析の修正}」のところで見たように，jamoviの分析結果はデータに修正を加えると分析結果も自動的に更新されるようになっています。SPSSなどの統計ソフトでは，一度実行した分析の結果は変更できません。そのため，分析の設定を変更したい場合には，再度分析をやり直すしかありませんし，データに修正が生じた場合も分析をやり直すしかないのですが，jamoviならそうした手間は不要です。
...
\caption{棒グラフ}\label{fig:Exploration-descriptives-plots-bar-plot}
\end{figure}

\hypertarget{subsub:exp-plot-continuous}{%
\subsubsection*{連続型データの視覚化}\label{subsub:exp-plot-continuous}}
\addcontentsline{toc}{subsubsection}{連続型データの視覚化}
...
\hypertarget{subsubsub:exp-plots-histograms}{%
\subsubsection*{Histograms：ヒストグラム}\label{subsubsub:exp-plots-histograms}}
\addcontentsline{toc}{subsubsection}{Histograms：ヒストグラム}

「\textbf{Histograms}」には，「Histogram（ヒストグラム）」と「Density（密度曲線）」の2つの項目があります。この2つは，組み合わせて使用することもできます。図\ref{fig:Exploration-descriptives-plots-histograms}は，「体重」のヒストグラムと密度曲線，その2つを組み合わせた場合の結果のグラフです\footnote{縦軸の「男」，「女」の部分が日本語だとうまく表示されず，□になってしまっているかもしれません。その場合の選択肢は，変数ラベルを「M」や「F」などのアルファベットに変更するか，これはこういうものだと思ってあきらめるかのいずれかです。}。ここでは性別をグループ変数として用いているので，男女別に図が作成されています。
...
\addcontentsline{toc}{paragraph}{Histogram：ヒストグラム}
...
\addcontentsline{toc}{paragraph}{Density：密度曲線}
...
\addcontentsline{toc}{subsubsection}{Q-Q Plots：Q-Qプロット}
...
\addcontentsline{toc}{subsubsection}{Box Plots：箱ひげ図}
...
\addcontentsline{toc}{paragraph}{Box plot：箱ひげ図}
...
\addcontentsline{toc}{paragraph}{Violin：バイオリン図}
...
\addcontentsline{toc}{paragraph}{Data：データ点}
...
\addcontentsline{toc}{subsubsection}{Bar Plots：棒グラフ}
...
\hypertarget{ch:ttests}{%
\chapter{T-Tests：ｔ検定}\label{ch:ttests}}
...
\caption{T-Tests（t検定）のメニュー}\label{fig:ttests-ttest-menu}
\end{figure}
...
これらの分析メニューでは，平均値の検定だけでなく，それぞれに対応した\emph{ノンパラメトリック検定}を行うこともできます。また，本書では詳しくはとりあげませんが，\emph{ベイズ統計}の考え方を用いた指標を算出することも可能です。

\hypertarget{sec:ttests-independent-samples}{%
\section{Independent Samples T-Test：対応なしt検定}\label{sec:ttests-independent-samples}}
...
\item
  \texttt{ID}　対象者のID
\item
  \texttt{グループ}　対象者のグループ（\texttt{1}または\texttt{2}）
\item
  \texttt{得点}　対象者の課題得点
\end{itemize}
...
\caption{対応なしt検定の実行}\label{fig:ttests-independent-menu}
\end{figure}
...
\caption{対応なしt検定の分析設定画面}\label{fig:ttests-independent-analysis}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{Test：検定}
...
\addcontentsline{toc}{paragraph}{Student's：スチューデントの検定}
...
\addcontentsline{toc}{paragraph}{Welch's：ウェルチの検定}
...
\addcontentsline{toc}{paragraph}{Mann-Whitney U：マン=ホイットニーのU}
...
\addcontentsline{toc}{subsubsection}{Hypothesis：仮説}
...
\addcontentsline{toc}{subsubsection}{Missing values：欠損値}
...
\addcontentsline{toc}{subparagraph}{Exclude cases analysis by analysis：分析ごとに除外}
...
\addcontentsline{toc}{subparagraph}{Exclude cases listwise：行全体を除外}
...
\addcontentsline{toc}{subsubsection}{Additional Statistics：追加の統計量}
...
\addcontentsline{toc}{paragraph}{Mean difference：平均値の差}
...
\addcontentsline{toc}{paragraph}{Effect size：効果量}
...
\addcontentsline{toc}{paragraph}{Descriptives：記述統計量}
...
\addcontentsline{toc}{paragraph}{Descriptives plots：記述統計量のグラフ}
...
\addcontentsline{toc}{subsubsection}{Assumption Checks：前提チェック}
...
\addcontentsline{toc}{paragraph}{Homogeneity test：等質性検定}
...
\addcontentsline{toc}{paragraph}{Normality test：正規性検定}
...
\addcontentsline{toc}{paragraph}{Q-Q plot：Q-Qプロット}
...
\hypertarget{subsec:ttests-paired-analysis}{%
\subsection{分析手順}\label{subsec:ttests-paired-analysis}}

ここでは次のサンプルデータ（\href{https://github.com/sbtseiji/jmv_compguide/raw/main/data/omv/ttests_data02.omv}{ttests\_data02.omv}）を用いて対応ありt検定の実施方法を見ていきましょう。このデータは，20名の参加者を対象に，効率的な記憶法の訓練前と訓練後における記憶課題の成績を測定したものです。このデータファイルには，次の3つの変数値が格納されています（図\ref{fig:ttests-data02}）。
...
\caption{サンプルデータ}\label{fig:ttests-data02}
\end{figure}
...
\item
  \texttt{ID}　対象者のID
\item
  \texttt{訓練前}　訓練前の記憶課題成績
\item
  \texttt{訓練後}　訓練後の記憶課題成績
\end{itemize}
...
\caption{対応ありt検定の実行}\label{fig:ttests-paired-t-menu}
\end{figure}
...
\caption{変数ペアの設定}\label{fig:ttests-paired-variables}
\end{figure}
...
\caption{対応ありt検定の分析結果}\label{fig:ttests-paired-results}
\end{figure}

変数名の代わりに変数ペアが表示されること以外は，分析結果の表も対応なしt検定の場合と同じです。この分析結果では，有意確率pの値が「\textless.001（0.001未満）」となっていますので，検定結果は有意，つまり記憶法の訓練前後で平均値に有意な差があるということになります。
...
\addcontentsline{toc}{subsubsection}{Tests：検定}
...
\caption{1標本t検定の実行}\label{fig:ttests-one-menu}
\end{figure}
...
\caption{1標本t検定の分析設定画面}\label{fig:ttests-one-setting}
\end{figure}
...
\hypertarget{sub:ttest-one-paired}{%
\subsection{1標本t検定と対応ありt検定}\label{sub:ttest-one-paired}}
...
そのことをここで確かめておきましょう。対応ありt検定のところで使用したサンプルデータ（\href{https://github.com/sbtseiji/jmv_compguide/raw/main/data/omv/ttests_data02.omv}{ttests\_data02.omv}）を再度開いてください（図\ref{fig:ttests-data02a}）。
...
\caption{対応ありt検定のサンプルデータ}\label{fig:ttests-data02a}
\end{figure}
...
\caption{計算変数を追加}\label{fig:ttests-append-compvar}
\end{figure}

次に，作成した計算変数の変数名の部分「\infig{variable-nominal.pdf} D」をダブルクリックして，変数設定画面を表示します。変数名を「変化量」に変更し，計算式の部分に「\texttt{訓練前\ -\ 訓練後}」と入力してください。マイナス記号（\texttt{-}）を入力する際は，日本語入力がオフになっていることを確認しましょう（図\ref{fig:ttests-set-compvar}）。
...
\caption{計算変数の設定}\label{fig:ttests-set-compvar}
\end{figure}
...
\caption{作成したDIFF変数}\label{fig:ttests-compvar-diff}
\end{figure}
...
\caption{1標本t検定を実行}\label{fig:ttests-diff-one}
\end{figure}

設定画面で，先ほど作成した「変化量」をDependent Variables（従属変数）に設定します（図\ref{fig:ttests-diff-one-setting}
）。対応ありt検定では，2グループの差の平均値が0であるかどうかについての検定を行いますので，「\textbf{Hypothesis}」の「Test value」は0のままで構いません。
...
\caption{分析の設定}\label{fig:ttests-diff-one-setting}
\end{figure}
...
\caption{分析結果}\label{fig:ttests-one-paired-results}
\end{figure}
...
\chapter{ANOVA：分散分析}\label{ch:ANOVA}}
...
\caption{ANOVA（分散分析）のメニュー}\label{fig:ANOVA-menu}
\end{figure}
...
\item
  MANCOVA（多変量共分散分析）
\item
  \textbf{Non-Parametric}（ノンパラメトリック検定）
...
\caption{まずは記述統計から}\label{fig:ANOVA-oneway-descriptives}
\end{figure}
...
\hypertarget{sub:ANOVA-oneway-procedure}{%
\subsection{分析手順}\label{sub:ANOVA-oneway-procedure}}
...
\caption{1要因分散分析の実行}\label{fig:ANOVA-oneway-menu}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{Variances：分散}
...
\addcontentsline{toc}{paragraph}{Don't assume equal (Welch's)：ウェルチの検定}
...
\addcontentsline{toc}{paragraph}{Assume equal (Fisher's)：フィッシャーの検定}
...
\addcontentsline{toc}{paragraph}{Normlity test：正規性検定}
...
\addcontentsline{toc}{subsubsection}{Post-Hoc Test：事後検定}
...
\addcontentsline{toc}{subparagraph}{Games-Howell (unequal variances)：ゲームス=ハウエル法}
...
\addcontentsline{toc}{subparagraph}{Tukey (equal variances)：テューキー法}
...
\addcontentsline{toc}{subsubsection}{Statistics：統計量}
...
\addcontentsline{toc}{paragraph}{Report significance：有意確率の報告}
...
\addcontentsline{toc}{paragraph}{Flag significant compariso：差が有意なペアにマーク}
...
\caption{各条件の平均値を算出}\label{fig:ANOVA-anova-descriptives}
\end{figure}
...
\caption{\label{tab:ANOVA-anova-marginal-means}各条件の平均値のまとめ}
\centering
...
\hypertarget{sub:ANOVA-anova-procedure}{%
\subsection{分析手順}\label{sub:ANOVA-anova-procedure}}
...
\caption{分散分析の実行}\label{fig:ANOVA-anova-menu}
\end{figure}
...
\caption{分散分析の設定画面}\label{fig:ANOVA-anova-settings}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{Model Fit：モデル適合度}
...
\addcontentsline{toc}{subsubsection}{Effect Size：効果量}
...
\addcontentsline{toc}{subsubsection}{Model Terms：モデル項}
...
\caption{分析モデルから項目を除外}\label{fig:ANOVA-anova-model-2}
\end{figure}
...
\caption{項目追加用のポップアップメニュー}\label{fig:ANOVA-anova-model-popup}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{Sum of squares：2乗和のタイプ}
...
\addcontentsline{toc}{subparagraph}{Type 1：タイプ1}
...
\addcontentsline{toc}{subparagraph}{Type 3：タイプ3}
...
\addcontentsline{toc}{subparagraph}{Type 2：タイプ2}
...
前提チェックの下にある\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Contrasts}}（コントラスト）を展開すると，次のような画面が表示されます。この画面では，各主効果の\emph{対比}の設定を行うことができます（図\ref{fig:ANOVA-anova-contrasts}）。
...
\addcontentsline{toc}{subparagraph}{deviation（偏差対比）}
...
\caption{水準の基準レベルを変更}\label{fig:ANOVA-anova-contrast-level-edit}
\end{figure}

すると，分散分析結果の対比の表の部分にその変更が反映されて，「なし」の平均値と全体の平均値との対比の結果が表示されます。差の値を確認できたら，水準の順番は元に戻しておきましょう。

\hypertarget{simpleux5358ux7d14ux5bfeux6bd4}{%
\subparagraph*{simple（単純対比）}\label{simpleux5358ux7d14ux5bfeux6bd4}}
\addcontentsline{toc}{subparagraph}{simple（単純対比）}

2つ目の\emph{単純対比}は，それぞれの要因における基準レベルの平均値と，それ以外の水準の平均値との比較です。この対比は，各実験群の平均値が統制群と比べて高い（低い）といえるかどうかに関心がある場合などに使用される方法です（図\ref{fig:ANOVA-anova-contrast-simple}）。
...
\caption{単純対比の結果}\label{fig:ANOVA-anova-contrast-simple}
\end{figure}
...
\addcontentsline{toc}{subparagraph}{difference（差分対比）}
...
\addcontentsline{toc}{subparagraph}{helmert（ヘルマート対比）}
...
\addcontentsline{toc}{subparagraph}{repeated（反復対比）}
...
\addcontentsline{toc}{subparagraph}{polynomial（多項式対比）}
...
\addcontentsline{toc}{subsubsection}{Correction：修正}
...
\addcontentsline{toc}{paragraph}{No correction：修正なし}
...
\addcontentsline{toc}{paragraph}{Tukey：テューキー法}
...
\addcontentsline{toc}{paragraph}{Scheffe：シェフェ法}
...
\addcontentsline{toc}{paragraph}{Bonferroni：ボンフェロニ法}
...
\addcontentsline{toc}{paragraph}{Holm：ホルム法}
...
\addcontentsline{toc}{subsubsection}{Output：出力}
...
\addcontentsline{toc}{subsubsection}{General Options：全般オプション}
...
\addcontentsline{toc}{paragraph}{Equal cell weights：均等重みづけ}
...
\addcontentsline{toc}{paragraph}{Confidence interval：信頼区間}
...
\addcontentsline{toc}{subsubsection}{Plot：作図}
...
\addcontentsline{toc}{paragraph}{Error bars：誤差線}
...
\addcontentsline{toc}{paragraph}{Observed scores：観測値}
...
\hypertarget{sub:ANOVA-rep-procedure}{%
\subsection{分析手順}\label{sub:ANOVA-rep-procedure}}
...
\caption{反復測定分散分析の実行}\label{fig:ANOVA-rm-menu}
\end{figure}
...
\addcontentsline{toc}{paragraph}{η²：イータ2乗}
...
\addcontentsline{toc}{paragraph}{Partial η²：偏イータ2乗}
...
\addcontentsline{toc}{paragraph}{Generalised η²：一般化イータ2乗}
...
\addcontentsline{toc}{subsubsection}{Dependent Variable Label：従属変数ラベル}
...
\addcontentsline{toc}{paragraph}{Sphericity tests：球面性検定}
...
\addcontentsline{toc}{subsubsection}{Sphericity corrections：球面性補正}
...
\addcontentsline{toc}{paragraph}{None（なし）}
...
\addcontentsline{toc}{paragraph}{Greenhouse-Geisser（グリーンハウス=ガイザー補正）}
...
\addcontentsline{toc}{paragraph}{Huynh-Feldt（ヒューン=フェルト補正）}
...
この\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Post Hoc Tests}}（事後検定）の設定項目は「ANOVA」の場合とまったく同じですので，ここでは説明を省略します。詳細については「ANOVA」の「\protect\hyperlink{sub:ANOVA-anova-posthoc}{Post Hoc Tests：事後検定}」の部分を見てください。

\hypertarget{sub:ANOVA-rm-marginal}{%
\subsection{Estimated Marginal Means：推定周辺平均}\label{sub:ANOVA-rm-marginal}}

この\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Estimated Marginal Means}}（推定周辺平均）の項目も，設定項目自体は「ANOVA」の場合と同じです。ただし，推定周辺平均のグラフを作成した場合には，縦軸（数値軸）のラベルとして「\textbf{Dependent Variable Label}（従属変数ラベル）」に設定した名前が使用されます（図\ref{fig:ANOVA-rm-emm-plot}）。
...
\item
  \texttt{ID}　参加者のID
\item
  \texttt{作業記憶}　参加者の作業記憶の容量
\item
  \texttt{BGM}　学習時のBGM（なし，音楽のみ，歌詞あり）
\item
  \texttt{再生数}　無意味つづりの再生数
\end{itemize}
...
\caption{作業記憶容量の記述統計}\label{fig:ANOVA-ancova-descriptives}
\end{figure}
...
\caption{作業記憶容量の平均値と標準偏差}\label{fig:ANOVA-ancova-means}
\end{figure}
...
「\infig{analysis-regression.pdf} Regression（回帰）」から「Correlation Matrix（相関行列）」を選択し，「作業記憶」と「再生数」の2つを指定してこの2つの間の相関係数を算出します（図\ref{fig:ANOVA-ancova-correlation}）。「\infig{analysis-regression.pdf} Regression（回帰）」ツールや相関係数の算出方法についての詳細は，第\ref{ch:regression}章の「\protect\hyperlink{ch:regression}{回帰分析}」を参照してください。
...
\caption{相関係数の算出}\label{fig:ANOVA-ancova-correlation}
\end{figure}

結果を見てみると，「作業記憶」と「再生数」の間の相関係数は0.62で，やや強めの正の相関があります（図\ref{fig:ANOVA-ancova-cor-results}）。つまりこれは，作業記憶の容量が多い人ほど，再生数が多いという関係があるということです。このように，分析における従属変数（再生数）に対して系統的な影響を与える変数（作業記憶の容量）のことを\emph{共変量}と呼びます。
...
\caption{「作業記憶」と「再生数」の相関係数}\label{fig:ANOVA-ancova-cor-results}
\end{figure}
...
\hypertarget{sub:ANOVA-ancova-procedure}{%
\subsection{分析手順}\label{sub:ANOVA-ancova-procedure}}
...
\caption{共分散分析の実行}\label{fig:ANOVA-ancova-menu}
\end{figure}
...
\caption{共分散分析の設定画面}\label{fig:ANOVA-ancova-settings}
\end{figure}
...
さて，先ほどから何度も繰り返しているように，この「ANCOVA」の設定項目は，共変量の設定欄がある以外は「ANOVA」のものと同一です。そのため，\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Model}}や\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Assumption Checks}}などの設定についての個別の説明はここでは省略します。それらについては「\protect\hyperlink{sec:ANOVA-anova}{ANOVA}」の該当箇所を参照してください。
...
\caption{3つの分布を別の視点から眺めた場合}\label{fig:ANOVA-mancova-multivariate4}
\end{figure}
...
\hypertarget{sub:ANOVA-mancova-procedure}{%
\subsection{分析手順}\label{sub:ANOVA-mancova-procedure}}
...
\caption{多変量共分散分析の実行}\label{fig:ANOVA-mancova-menu}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{Multivariate Statistics：多変量統計量}
...
\addcontentsline{toc}{paragraph}{Pillai's Trace：ピライのトレース}
...
\addcontentsline{toc}{paragraph}{Wilks's Lambda：ウィルクスのラムダ}
...
\addcontentsline{toc}{paragraph}{Hotelling's Trace：ホテリングのトレース}
...
\addcontentsline{toc}{paragraph}{Roy's Largest Root：ロイの最大根}
...
\addcontentsline{toc}{paragraph}{Box's M test：ボックスのM検定}
...
\addcontentsline{toc}{paragraph}{Shapiro-Wilk test：シャピロ=ウィルク検定}
...
\addcontentsline{toc}{paragraph}{Q-Q plot of multivariate normality：多変量正規Q-Qプロット}
...
\caption{フィルタを選択}\label{fig:ANOVA-mancova-posthoc}
\end{figure}
...
式の途中の「イコール」は，「\texttt{=}」でなく「\texttt{==}」と2つ続ける必要があるので注意してください。また，変数名は，関数ボタン（\infig{filters-formula.pdf}）を使って入力するのが確実ですが，キーボードから入力しても問題ありません。
...
\caption{フィルタの設定}\label{fig:ANOVA-mancova-filters1}
\end{figure}
...
\caption{フィルタの追加}\label{fig:ANOVA-mancova-filters2}
\end{figure}
...
\caption{「教室」と「オンデマンド」の比較用フィルタ}\label{fig:ANOVA-mancova-filters3}
\end{figure}
...
\caption{クラスカル=ウォリス検定の実行}\label{fig:ANOVA-kw-menu}
\end{figure}
...
\caption{クラスカル=ウォリス検定の設定画面}\label{fig:ANOVA-kw-settings}
\end{figure}
...
\addcontentsline{toc}{paragraph}{DSCF pairwise comparisons：多重比較}
...
\hypertarget{sub:ANOVA-friedman-procedure}{%
\subsection{分析手順}\label{sub:ANOVA-friedman-procedure}}
...
\caption{フリードマン検定の実行}\label{fig:ANOVA-friedman-menu}
\end{figure}
...
\caption{フリードマン検定の設定画面}\label{fig:ANOVA-friedman-settings}
\end{figure}
...
\addcontentsline{toc}{paragraph}{Pairwise comparisons (Durbin-Conover)：多重比較}
...
\addcontentsline{toc}{paragraph}{Descriptive plot：記述統計量のグラフ}
...
\chapter{Regression：回帰分析}\label{ch:regression}}
...
\caption{Regression（回帰分析）のメニュー}\label{fig:regression-menu}
\end{figure}
...
\item
  \textbf{Logistic Regression}（ロジスティック回帰）
...
\caption{相関係数の算出}\label{fig:regression-correlation-menu}
\end{figure}
...
\caption{相関行列の設定画面}\label{fig:regression-correlation}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{Correlation Coefficients：相関係数}
...
\addcontentsline{toc}{paragraph}{Spearman：スピアマンの順位相関係数}
...
\addcontentsline{toc}{paragraph}{Kendall's tau-b：ケンドールの順位相関係数}
...
\addcontentsline{toc}{subsubsection}{Additional Options：追加オプション}
...
\addcontentsline{toc}{paragraph}{Report significance：有意性を報告}
...
\addcontentsline{toc}{paragraph}{Flag significant correlations：有意な相関をマーク}
...
\addcontentsline{toc}{paragraph}{N：標本サイズ}
...
\addcontentsline{toc}{paragraph}{Confidence intervals：信頼区間}
...
\addcontentsline{toc}{paragraph}{Correlation matrix：相関行列}
...
\addcontentsline{toc}{paragraph}{Densities for variables：変数の密度曲線}
...
\addcontentsline{toc}{paragraph}{Statistics：統計量}
...
部分相関係数は，\emph{重回帰分析}においてその変数を分析モデルに含めるべきかどうかを判断する際などに，偏相関係数は他の変数の影響を取り除いた後の2変数間の関係について知りたい場合などに用いられます。

\hypertarget{sub:regression-partial-analysis}{%
\subsection{分析手順}\label{sub:regression-partial-analysis}}

ここでは，相関行列のところで用いたのと同じサンプルデータ（\href{https://github.com/sbtseiji/jmv_compguide/raw/main/data/omv/regression_data01.omv}{regression\_data01.omv}）を用いて，部分相関係数・偏相関係数を算出してみましょう。部分相関係数や偏相関係数の算出には，Analysesタブの「\infig{analysis-regression.pdf} Regression」から「Partial Correlation（偏相関）」を選択します（図\ref{fig:regression-partial-menu}）。
...
\caption{部分相関係数・偏相関係数の算出}\label{fig:regression-partial-menu}
\end{figure}
...
\caption{部分相関係数・偏相関係数の設定画面}\label{fig:regression-partial-setting}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{Correlation Type：相関タイプ}
...
\addcontentsline{toc}{subparagraph}{Partial：偏相関}
...
\addcontentsline{toc}{subparagraph}{Semipartial：部分相関}
...
\hypertarget{lr-uni}{%
\subsubsection*{単回帰}\label{lr-uni}}
\addcontentsline{toc}{subsubsection}{単回帰}
...
\caption{線形回帰の分析設定}\label{fig:regression-LR-analysis}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{重回帰}
...
ここでまず，「\textbf{Block 1}」のところにある「母身長」を選択し，「\infig{action-back.pdf}」をクリックしてBlock 1の部分から「母身長」を削除します（図\ref{fig:regression-lr-model-builder-mother}）。
...
\caption{motherをBlock 1から削除}\label{fig:regression-lr-model-builder-mother}
\end{figure}
...
\caption{新たなブロックを作成}\label{fig:regression-lr-model-builder-newblock}
\end{figure}

そして，先ほどBlock 1から削除した「母身長」を，新しく作成した「\textbf{Block 2}」にマウスでドラッグして移動します（図\ref{fig:regression-lr-model-builder-add-mother}）。
...
\caption{motherをBlock 2に追加}\label{fig:regression-lr-model-builder-add-mother}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{交互作用を用いた回帰分析}
...
そして，画面左側の「Predictors」の部分で「父身長」と「母身長」の2つを選択します\footnote{複数の変数を同時に選択するには，いずれかの変数をクリックして選択したのち，「SHIFT(⇧)」キーを押しながらもう一方の変数名をクリックしてください。}。そして「Predictors」と「Blocks」の間にある2つの「\infig{action-forward.pdf}」ボタンのうち，下にあるほうをクリックすると，図\ref{fig:regression-lr-model-builder-interaction-add}のように選択メニューが表示されますので，そこから「Interaction（交互作用）」を選択してください。
...
\caption{モデルビルダーで3つ目のブロックを作成}\label{fig:regression-lr-model-builder-interaction-add}
\end{figure}
...
\caption{3つ目のブロックに交互作用を設定}\label{fig:regression-lr-model-builder-interaction-setting2}
\end{figure}
...
\caption{連続型説明変数の設定}\label{fig:regression-lr-reference01}
\end{figure}
...
\caption{名義型説明変数の設定}\label{fig:regression-lr-reference02}
\end{figure}
...
今回の分析ではどちらの表現であっても構わないと思いますが，もし「男の子の身長」のほうに関心があって，それに対する説明変数の影響を知りたい場合には，この結果のような「女の子の場合，男の子に比べて身長が平均で0.96cm高くなる」という形よりも，「男の子の場合，女の子に比べて\ldots\ldots」という表現のほうがわかりやすいでしょう。そのような場合には，分析設定画面の\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Reference Levels}}（基準レベル）の項目で，基準となる変数値を指定することができます。
...
\addcontentsline{toc}{subsubsection}{Intercept：切片}

なお，\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Reference Levels}}の下のほうにある「\textbf{Intercept}（切片）」の部分では，名義型変数を回帰分析に用いる際の処理方法についての設定が可能です。，一般に，回帰モデルの説明変数に「性別」のような名義型の変数が含まれている場合には，基準とする値を「0」，比較する側の値を「1」に置き換える\emph{ダミーコード化}（\emph{ダミーコーディング}）という方法がとられます。たとえば，係数を算出する際の基準が「男」であれば，性別の値が「男」の場合を「0」，「女」の場合を「1」と置き換えたうえで計算を行うのです\footnote{3種類以上の値が含まれる名義型変数の場合には少し複雑になりますが，基本的な考え方はこれと同じです。}。
...
\addcontentsline{toc}{subparagraph}{Reference level (dummy coding)：ダミーコード}
...
\addcontentsline{toc}{subparagraph}{Grand mean (simple coding)：シンプルコード}
...
\addcontentsline{toc}{paragraph}{Autocorrelation test：自己相関検定}
...
\addcontentsline{toc}{paragraph}{Collinearity statistics：共線性統計量}
...
\addcontentsline{toc}{paragraph}{Q-Q plot of residuals：残差Q-Qプロット}
...
\addcontentsline{toc}{paragraph}{Residual plots：残差プロット}
...
\addcontentsline{toc}{subsubsection}{Data Summary：データ要約}
...
\addcontentsline{toc}{paragraph}{Cook's distance：クックの距離}
...
表として結果に示されるのは，各測定値についてのクックの距離の記述統計量（平均値，中央値と標準偏差，および最小値・最大値）のみですが，この後に触れる\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Save}}（\protect\hyperlink{sub:regression-LR-save}{保存}）で設定を行うことによって，各測定値についてのクックの距離を新たな変数として保存することができます。クックの距離を新たな変数として保存すれば，分析結果に悪影響を及ぼしているデータ行がどれであるかを特定することもできるようになります。
...
\addcontentsline{toc}{subsubsection}{Fit Measures：適合度指標}
...
\addcontentsline{toc}{paragraph}{Adjusted R²：調整済決定係数}
...
\addcontentsline{toc}{paragraph}{AIC，BIC，RMSE：モデル選択の指標}
...
\addcontentsline{toc}{subsubsection}{Overall Model Test：モデル全体の検定}
...
\addcontentsline{toc}{subsubsection}{Omnibus Test：オムニバス検定}
...
\addcontentsline{toc}{subsubsection}{Estimate：推定値}
...
\addcontentsline{toc}{subsubsection}{Standardized Estimate：標準化推定値}
...
\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Estimated Marginal Means}}（推定周辺平均）の設定項目では，\emph{推定周辺平均値}の算出や表示に関する設定を行います（図\ref{fig:regression-lr-emm}）。
...
\caption{2項ロジスティック回帰の設定画面}\label{fig:regression-binomial-setting}
\end{figure}
...
\caption{目的変数と説明変数の設定}\label{fig:regression-binomial-variable-settings}
\end{figure}
...
1つ目（図\ref{fig:regression-binomial-results}の左側）の表は，この回帰モデルの適合度指標（Model Fit Measures）に関する情報です。2項ロジスティック回帰では，初期設定では適合度の指標として\textbf{Deviance}（\emph{逸脱度}），\textbf{AIC}，\textbf{\(\textsf{R}^{\textsf{2}}_{\textsf{McF}}\)}（マクファデンのR²）の3つの値が表示されます。逸脱度およびAICは小さい値であるほど，R²の値は1に近い値ほど分析モデルのあてはまりがよいことを意味します。これらの適合度指標については，この後の\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Model Fit}}（モデル適合度）の部分で説明します。
...
先ほどは係数の値を指数変換することでオッズ比を求めましたが，実際にはこのような計算を自分でやる必要はありません。\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Model Coefficients}}の設定画面で「\textbf{Odds Ratio}（オッズ比）」の部分にチェックを入れれば，各回帰係数のオッズ比とその信頼区間を算出することができます。
...
\addcontentsline{toc}{paragraph}{Deviance：逸脱度}
...
\addcontentsline{toc}{paragraph}{AIC, BIC}
...
\addcontentsline{toc}{paragraph}{Overall model test：モデル全体の検定}
...
\addcontentsline{toc}{subsubsection}{Pseudo R²：擬似決定係数}
...
\addcontentsline{toc}{paragraph}{McFadden's R²：マクファデンのR²}
...
\addcontentsline{toc}{paragraph}{Cox \& Snell's R²：コックス=スネルのR²}
...
\addcontentsline{toc}{paragraph}{Nagelkerke's R²：ナゲルケルケのR²}
...
\addcontentsline{toc}{subsubsection}{Omnibus Tests：オムニバス検定}
...
\addcontentsline{toc}{subsubsection}{Estimate (Log Odds Ratio)：推定値（対数オッズ比）}
...
\addcontentsline{toc}{subsubsection}{Odds Ratio：オッズ比}
...
\addcontentsline{toc}{subsubsection}{Cut-Off：カットオフ}
...
\addcontentsline{toc}{subsubsection}{Predictive Measures：予測指標}
...
\addcontentsline{toc}{paragraph}{Classification table：分類表}
...
\addcontentsline{toc}{paragraph}{Accuracy, Specificity, Sensitivity：精度，特異度，感度}

その下にある「Accuracy」，「Specificity」，「Sensitivity」にチェックを入れると，それぞれ精度，特異度，感度の値が表示されます。

\hypertarget{roc}{%
\subsubsection*{ROC}\label{roc}}
\addcontentsline{toc}{subsubsection}{ROC}
...
\addcontentsline{toc}{paragraph}{ROC curve：ROC曲線}
...
\hypertarget{sub:regression-multinomial-analysis}{%
\subsection{分析手順}\label{sub:regression-multinomial-analysis}}
...
\caption{多項ロジスティック回帰分析の実行}\label{fig:regression-multinomial-menu}
\end{figure}
...
このとき，順序ロジスティックモデルでは，偏回帰係数（\(b_1\)，\(b_2\)，\(b_3\)）はすべてのモデルで同じであり，それぞれのモデルで異なるのは切片（\(b_{10}\)，\(b_{20}\)）と残差のみであるという仮定のもとで回帰係数の算出を行います。このような形で計算を行うロジスティック回帰モデルは\emph{累積ロジスティック回帰モデル}とも呼ばれます。

\hypertarget{sub:regression-ordinal-analysis}{%
\subsection{分析手順}\label{sub:regression-ordinal-analysis}}
...
\caption{順序ロジスティック回帰分析の実行}\label{fig:regression-ordinal-menu}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{モデルの閾値}
...
また，順序ロジスティック回帰のモデル係数の表には切片の推定値が含まれていません。この分析における切片の推定値を表示させるには，分析設定画面の\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Model Coefficients}}（モデル係数）の「\textbf{Thresholds}（閾値）」ところで「Model thresholds（モデルの閾値）」にチェックを入れます（図\ref{fig:regression-ordinal-thresholds}）。
...
また，各モデルの閾値（切片）の推定値についての検定では，「普通 \textbar{} 良い」は有意ですが「悪い \textbar{} 普通」は有意ではありませんでした。これは，「普通 \textbar{} 良い」つまり「普通以下（悪い・普通）」と「良い」の区別はできていても，「悪い」と「普通以上（普通・良い）」の区別はできていないということです。

これらの結果を総合すると，日中にたくさん歩くことは起床時の気分の良さにつながるけれども，歩かなかったからといって起床時の気分が悪くなるわけではないということになります。

\hypertarget{ch:frequencies}{%
\chapter{Frequencies：度数分析}\label{ch:frequencies}}
...
\caption{Frequencies（度数分析）のメニュー}\label{fig:frequencies-menu}
\end{figure}
...
\caption{サンプルデータ}\label{fig:frequencies-data01}
\end{figure}
...
\tightlist
\item
  \texttt{結果}　課題の結果（通過，非通過）
\item
  \texttt{人数}　通過・非通過の人数（通過：46人，非通過：34人）
\end{itemize}
...
\caption{2項検定の実行}\label{fig:frequencies-binomial-menu}
\end{figure}
...
\caption{2項検定の分析設定}\label{fig:frequencies-binomial-set-var}
\end{figure}
...
\hypertarget{hypothesisux4eeeux8aac-1}{%
\subsubsection*{Hypothesis：仮説}\label{hypothesisux4eeeux8aac-1}}
\addcontentsline{toc}{subsubsection}{Hypothesis：仮説}

「\textbf{Hypothesis}（仮説）」では，検定に使用する対立仮説の設定を行います。
...
\addcontentsline{toc}{subsubsection}{Prior：事前分布}

ベイズ統計の詳細についてはベイズ統計の教科書を参照して欲しいと思いますが，ベイズ統計では，分析対象についての確率分布をあらかじめ設定し，その分布に対して観測データを与えて分析結果を得るという手順で分析を実施します。この際，分析対象についてあらかじめ設定する分布のことを\emph{事前分布}と呼びます。\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Baysian Statistics}}の「\textbf{Prior}（事前分布）」の部分は，この事前分布についての設定です。
...
\addcontentsline{toc}{subsubsection}{Plots：作図}
...
\caption{適合度検定の実行}\label{fig:frequencies-n-outcomes-goodness}
\end{figure}
...
\caption{適合度検定の設定画面}\label{fig:frequencies-n-outcomes-goodness-setting}
\end{figure}
...
\caption{\label{tab:frequencies-cake-table2}性別とケーキの選択の期待度数}
\centering
\begin{tabular}[t]{lcccc}
\toprule
\multicolumn{1}{c}{ } & \multicolumn{3}{c}{ケーキの種類} & \multicolumn{1}{c}{ } \\
...
\hypertarget{sub:frequencies-chisq-procedure}{%
\subsection{分析手順}\label{sub:frequencies-chisq-procedure}}
...
このようなデータの場合，ケーキの選択と性別の関連を適切に把握するために，分析対象のクロス集計表を大人と子供にグループ分けしたうえで分析するという方法がとられます。このようにしてクロス集計表を別のグループ変数で分割して分析する方法は，\emph{層別分析}と呼ばれます。jamoviでは，この層別分析についても簡単に実行できるようになっています。

\hypertarget{subsub:frequencies-chisq-layer}{%
\subsubsection*{分析手順}\label{subsub:frequencies-chisq-layer}}
\addcontentsline{toc}{subsubsection}{分析手順}
...
\caption{層別分析の設定（その1）}\label{fig:frequencies-chisq-layer-set-var1}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{分析結果}
...
\addcontentsline{toc}{paragraph}{\(\chi^2\) continuity correction：連続性の修正}
...
\addcontentsline{toc}{paragraph}{Likelihood ratio：尤度比}
...
\addcontentsline{toc}{paragraph}{Fisher's exact test：フィッシャーの正確確率検定}
...
\addcontentsline{toc}{paragraph}{z test for difference in 2 proportions：比率の差のz検定}
...
\addcontentsline{toc}{subsubsection}{Comparative Measures：比較指標}
...
\addcontentsline{toc}{paragraph}{Odds ratio：オッズ比}
...
\addcontentsline{toc}{paragraph}{Log odds ratio：対数オッズ比}
...
\addcontentsline{toc}{paragraph}{Relative risk：リスク比}
...
\addcontentsline{toc}{paragraph}{Difference in proportions：比率の差}
...
\caption{比較指標の関係と一覧}\label{fig:frequencies-chisq-comparative-measures}
\end{figure}

\hypertarget{confidence-intervalsux4fe1ux983cux533aux9593-1}{%
\paragraph*{Confidence intervals：信頼区間}\label{confidence-intervalsux4fe1ux983cux533aux9593-1}}
\addcontentsline{toc}{paragraph}{Confidence intervals：信頼区間}

「Confidence intervals」の項目にチェックを入れると，オッズ比や対数オッズ比，リスク比などの各指標について，指定した幅の信頼区間が算出されます。

\hypertarget{nominalux540dux7fa9ux5c3aux5ea6}{%
\subsubsection*{Nominal：名義尺度}\label{nominalux540dux7fa9ux5c3aux5ea6}}
\addcontentsline{toc}{subsubsection}{Nominal：名義尺度}

「\textbf{Nominal}（名義尺度）」では，分析対象のデータが名義型変数（\infig{variable-nominal.pdf} Nominal）である場合の\textbf{連関係数}に関する設定を行います。連関係数とは，クロス集計表における行の要素と列の要素の間の関連の強さを表す指標のことです。
...
\addcontentsline{toc}{paragraph}{Contingency coefficient：コンティンジェンシー係数}
...
\addcontentsline{toc}{paragraph}{Phi and Cramer's V：ファイとクラメールのV}

この項目にチェックを入れると，\emph{ファイ係数}と\emph{クラメールの連関係数V}の2種類の値が算出されます。

ファイ（\(\varphi\)）係数と呼ばれるものには，\(\chi^2\)の値を総度数で割ったものの平方根として求められるものと，ピアソンの積率相関係数を2行2列のクロス集計表に適用したもの（これは\emph{四分点相関係数}とも呼ばれます）の2種類が存在しますが，jamoviで算出されるのは後者です。このファイ係数は，2行2列のクロス集計表でのみ算出されます。
...
\hypertarget{ordinalux9806ux5e8fux5c3aux5ea6}{%
\subsubsection*{Ordinal：順序尺度}\label{ordinalux9806ux5e8fux5c3aux5ea6}}
\addcontentsline{toc}{subsubsection}{Ordinal：順序尺度}

「\textbf{Ordinal}（順序尺度）」では，分析対象のデータが順序型変数（\infig{variable-ordinal.pdf} Ordinal）である場合の関連度指標に関する設定を行います。
...
\caption{順序尺度の関連度}\label{fig:frequencies-chisq-ordinal}
\end{figure}
...
\addcontentsline{toc}{paragraph}{Gamma：グッドマン=クラスカルのガンマ}
...
\addcontentsline{toc}{paragraph}{Mantel-Haenszel：マンテル=ヘンツェルの傾向性検定}
...
\caption{集計済みデータの入力方法}\label{fig:frequencies-data06}
\end{figure}
...
\caption{マクニマー検定の実行}\label{fig:frequencies-mcnemar-menu}
\end{figure}
...
\caption{マクニマー検定の設定画面}\label{fig:frequencies-mcnemar-settings}
\end{figure}
...
\caption{対数線形回帰分析の実行}\label{fig:frequencies-loglin-menu}
\end{figure}
...
\caption{対数線形回帰分析の設定画面}\label{fig:frequencies-loglin-settings}
\end{figure}
...
モデルビルダーの「\textbf{Block 2}」にある「性別＊年齢＊ケーキ」を選択し，「\infig{action-back.pdf}」をクリックして分析モデルから除外します（図\ref{fig:frequencies-loglin-modelbuilder-2way-removed}）。
...
\caption{分析モデルから交互作用を除外}\label{fig:frequencies-loglin-modelbuilder-2way-removed}
\end{figure}
...
そこで，今度はこの「性別＊年齢」を「\textbf{Block 2}」に入れて，これを除外した場合とそうでない場合とでモデルの適合度がどう変化するかを見てみましょう（図\ref{fig:frequencies-loglin-modelbuilder-compare2}）。
...
\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Estimated Marginal Means}}（推定周辺平均）の設定項目やその使用方法は，第\ref{ch:regression}章の線形回帰のところで説明したのと同じですので，詳細についてはここでは説明は省略します（線形回帰の「\protect\hyperlink{sub:regression-LR-marginalmeans}{Estimated Marginal Means（推定周辺平均）}」を参照）。

なお，このオプションで表示される「推定周辺平均」は，回帰モデルの予測値，つまりは度数の推定値ですので，その点だけ注意してください。

\hypertarget{ch:factor}{%
\chapter{Factor：因子分析}\label{ch:factor}}
...
\caption{Factor（因子分析）のメニュー}\label{fig:factor-menu}
\end{figure}
...
\item
  \texttt{ID}　対象者のID
\item
  \texttt{Q1}　「メッセージアプリをよく使う」への回答値
\item
  \texttt{Q2}　「メッセージアプリよりメールで連絡することのほうが多い」への回答値
\item
  \texttt{Q3}　「メッセージアプリでやりとりするのは楽しい」への回答値
\item
  \texttt{Q4}　「メッセージアプリは何かとめんどくさい」への回答値
\item
  \texttt{Q5}　「送信メッセージに反応がないとソワソワする」への回答値
\end{itemize}
...
\caption{信頼性分析の実行}\label{fig:factor-reliability-menu}
\end{figure}
...
\caption{信頼性分析の設定}\label{fig:factor-reliability-setting}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{Scale Statistics：尺度統計量}
...
\addcontentsline{toc}{paragraph}{McDonald's ω：マクドナルドのオメガ}
...
\addcontentsline{toc}{paragraph}{Mean，Standard deviation：平均値，標準偏差}
...
\addcontentsline{toc}{subsubsection}{Item Statistics：項目統計量}
...
\addcontentsline{toc}{paragraph}{Item-rest correlation：I-R相関}
...
\addcontentsline{toc}{subsubsection}{Method：回転}
...
\addcontentsline{toc}{subsubsection}{Number of Components：主成分の個数}
...
\addcontentsline{toc}{subparagraph}{Based on parallel analysis：平行分析を基準}
...
\addcontentsline{toc}{subparagraph}{Based on eigenvalue：固有値を基準}
...
\addcontentsline{toc}{subparagraph}{Fixed number：個数指定}
...
\addcontentsline{toc}{subsubsection}{Factor loadings：因子負荷}
...
\addcontentsline{toc}{paragraph}{Hide loadings below：小さな負荷量を隠す}
...
\addcontentsline{toc}{paragraph}{Sort loadings by size：負荷量の大きさで並び替え}
...
\addcontentsline{toc}{subsubsection}{Additional Output：追加の出力}
...
\addcontentsline{toc}{paragraph}{Component summary：主成分の要約}
...
\addcontentsline{toc}{paragraph}{Component correlations：主成分の相関}
...
\addcontentsline{toc}{paragraph}{Initial eigenvalues：回転前の固有値}
...
\addcontentsline{toc}{paragraph}{Scree plot：スクリープロット}
...
\addcontentsline{toc}{subsubsection}{Assumption Check：前提チェック}
...
\addcontentsline{toc}{paragraph}{Bartlett's test of sphericity：バートレットの球面性検定}
...
\addcontentsline{toc}{paragraph}{KMO measure of sampling adequacy：KMO標本妥当性指標}
...
\hypertarget{sub:factor-pca-score-analysis}{%
\subsection{主成分得点を用いた分析}\label{sub:factor-pca-score-analysis}}
...
\caption{主成分得点を用いたt検定}\label{fig:factor-pca-t-test-setting}
\end{figure}

また，男児それぞれにおける各主成分得点の平均値を見るために，「\textbf{Additional Statistics}（追加の統計量）」の部分で「Descriptives（記述統計）」にもチェックを入れておきましょう（図\ref{fig:factor-pca-ttest-descriptive}）。
...
\caption{t検定と同時に記述統計量を算出}\label{fig:factor-pca-ttest-descriptive}
\end{figure}
...
\caption{探索的因子分析の実行}\label{fig:factor-efa-menu}
\end{figure}
...
\caption{探索的因子分析の設定画面}\label{fig:factor-efa-setting}
\end{figure}
...
\addcontentsline{toc}{subsection}{Method：方法}
...
\addcontentsline{toc}{paragraph}{Extraction：因子抽出}
...
\addcontentsline{toc}{subparagraph}{Minimum residuals：最小残差法}
...
\addcontentsline{toc}{subparagraph}{Maximum likelihood：最尤法}
...
\addcontentsline{toc}{subparagraph}{Principal axis：主因子法}
...
\addcontentsline{toc}{paragraph}{Rotation：回転}
...
\addcontentsline{toc}{subparagraph}{None：回転なし}
...
\addcontentsline{toc}{subparagraph}{Varimax：バリマックス法}
...
\addcontentsline{toc}{subparagraph}{Quatimax：クォーティマックス法}
...
\addcontentsline{toc}{subparagraph}{Promax：プロマックス法}
...
\addcontentsline{toc}{subparagraph}{Oblimin：オブリミン法}
...
\addcontentsline{toc}{subparagraph}{Simplimax：シンプリマックス法}
...
\addcontentsline{toc}{subsubsection}{Number of Factors：因子の個数}
...
\addcontentsline{toc}{subsubsection}{Factor Loadings：因子負荷量}
...
\addcontentsline{toc}{paragraph}{Factor summary：因子の要約}
...
\addcontentsline{toc}{paragraph}{Factor correlations：因子の相関}
...
\addcontentsline{toc}{paragraph}{Model fit measures：モデル適合度指標}
...
\addcontentsline{toc}{subsubsection}{因子の解釈}
...
\hypertarget{sub:factor-cfa-procedure}{%
\subsection{分析手順}\label{sub:factor-cfa-procedure}}
...
\caption{確認的因子分析の実行}\label{fig:factor-cfa-menu}
\end{figure}
...
\caption{確認的因子分析の設定画面}\label{fig:factor-cfa-setting}
\end{figure}
...
\hypertarget{sub:factor-cfa-residual-covariances}{%
\subsection{Residual Covariances：残差共分散}\label{sub:factor-cfa-residual-covariances}}
...
\caption{残差共分散の設定}\label{fig:factor-cfa-residual-covariance}
\end{figure}

この\emph{残差共分散}（\emph{誤差共分散}とも呼ばれます）を算出するには，ここで残差の共分散を算出したい変数のペアを選択し，それを右側の「Residual Covariances（残差共分散）」の欄に移動します。なお，「\infig{analysis-regression.pdf} Regression（回帰分析）」メニューの「Correlation Matrix（相関行列）」で相関係数を算出する場合とは異なり，ここでは共分散を算出させたい変数のペアを個別に指定する必要があります（図\ref{fig:factor-cfa-residual-covariance2}）。
...
\caption{残差共分散の設定の例}\label{fig:factor-cfa-residual-covariance2}
\end{figure}
...
\addcontentsline{toc}{subsubsection}{欠損値の処理方法}
...
\addcontentsline{toc}{subsubsection}{推定における制約}
...
\addcontentsline{toc}{subsubsection}{Results：結果}
...
\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Estimates}}（推定値）右側の\textbf{Statistics}（統計量）の部分では，係数の推定に関する設定を行うことができます。1つ目の「Test statistics（検定統計量）」にチェックを入れると，算出した係数（分散や共分散も含む）の推定値について，その係数が0でないかどうかの有意性検定の結果が表示されます。確認的因子分析では，いずれの係数についても標準得点zを用いた検定が行われます。
...
\addcontentsline{toc}{subsubsection}{Test for Exact Fit：適合度検定}
...
\addcontentsline{toc}{paragraph}{CFI，TLI}
...
\addcontentsline{toc}{paragraph}{SRMR，RMSEA}
...
AICとBICについては，回帰分析の\colorbox{bar}{\raisebox{-.25\zh}{\includegraphics[height=1\zh]{images/assets/modules-analyses-arrow-r}}\textcolor{gmoji2}{| Model Fit}}（\protect\hyperlink{sub:regression-LR-modelfit}{モデル適合度}
...
\addcontentsline{toc}{subsubsection}{Post-Hoc Model Performance：モデルの事後修正}
...
\addcontentsline{toc}{paragraph}{Residuals observed correlation matrix：相関行列の残差}
...
\addcontentsline{toc}{paragraph}{Modification indices：修正指標}
...
\chapter*{関数一覧}\label{ch:appendix}}
\addcontentsline{toc}{chapter}{関数一覧}

\AppendixChapter{関数一覧}

\hypertarget{app:math}{%
\subsection*{Math：数学関数}\label{app:math}}
\addcontentsline{toc}{subsection}{Math：数学関数}

\begin{description}
\tightlist
\item[ABS(\texttt{値})]
\texttt{値}の絶対値を返します（\(\mathsf{|x|}\)）
\item[EXP(\texttt{値})]
ネイピア数eの\texttt{値}乗の値（eのべき乗）を返します（\(\mathsf{e^{x}}\)）
\item[LN(\texttt{値})]
\texttt{値}の自然対数を返します（\(\mathsf{\log_{e}{x}}\)）
\item[LOG10(\texttt{値})]
\texttt{値}の常用対数を返します（\(\mathsf{\log_{10}{x}}\)）
\item[SQRT(\texttt{値})]
\texttt{値}の正の平方根を返します（\(\mathsf{\sqrt{x}}\)）
\end{description}

\hypertarget{app:statistical}{%
\subsection*{Statistical：統計関数}\label{app:statistical}}
\addcontentsline{toc}{subsection}{Statistical：統計関数}
...
\addcontentsline{toc}{subsubsection}{V関数}
...
\addcontentsline{toc}{subsection}{Logical：論理関数}
...
\addcontentsline{toc}{subsection}{Text：文字列関数}
...
\addcontentsline{toc}{subsection}{Reference：参照関数}
...
\addcontentsline{toc}{subsection}{Misc：各種関数}
...
\addcontentsline{toc}{subsection}{Simulation：シミュレーション関数}
